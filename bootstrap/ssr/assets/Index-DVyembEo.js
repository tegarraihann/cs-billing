        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            if (showTopupModal.value) {
              ...
              _push2(`</div></div><div${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-1"${_scopeId}>Deskripsi</label><input${ssrRenderAttr("value", unref(topupForm).description)} type="text" class="w-full rounded-md border-gray-300 shadow-sm focus:border-sage-500 focus:ring-sage-500" placeholder="Contoh: Sewa kantor Q1"${_scopeId}></div><div${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-1"${_scopeId}> Biaya <span class="text-red-500"${_scopeId}>*</span></label><select class="w-full rounded-md border-gray-300 shadow-sm focus:border-sage-500 focus:ring-sage-500" required${_scopeId}><option value=""${ssrIncludeBooleanAttr(Array.isArray(unref(topupForm).pl_account_id) ? ssrLooseContain(unref(topupForm).pl_account_id, "") : ssrLooseEqual(unref(topupForm).pl_account_id, "")) ? " selected" : ""}${_scopeId}>Pilih biaya</option><!--[-->`);
              ssrRenderList(expenseAccounts.value, (account) => {
                _push2(`<option${ssrRenderAttr("value", account.id)}${ssrIncludeBooleanAttr(Array.isArray(unref(topupForm).pl_account_id) ? ssrLooseContain(unref(topupForm).pl_account_id, account.id) : ssrLooseEqual(unref(topupForm).pl_account_id, account.id)) ? " selected" : ""}${_scopeId}>${ssrInterpolate(account.account_code)} - ${ssrInterpolate(account.account_name)}</option>`);
              });
              _push2(`<!--]--></select>`);
