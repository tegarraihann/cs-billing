...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const payableStatus = computed(() => {
      var _a;
      return ((_a = payable.value) == null ? void 0 : _a.status) ?? summaryStatus.value ?? "unpaid";
    });
    const payableOutstanding = computed(() => {
      var _a;
      return Number(((_a = payable.value) == null ? void 0 : _a.outstanding_amount) ?? summary.value.total_outstanding ?? 0);
    });
    ...
    const canPostPph23Payable = computed(() => {
      var _a;
      return payableOutstanding.value > 0 && payableStatus.value !== "paid" && !((_a = payable.value) == null ? void 0 : _a.pph23_payable_posted_at);
    });
    const canPostVatReceivable = computed(() => {
      var _a;
      return payableStatus.value === "paid" && payableOutstanding.value <= 0 && !((_a = payable.value) == null ? void 0 : _a.vat_receivable_posted_at);
    });
                                    ...
                                    isVatMenuOpen(component.id) ? (openBlock(), createBlock("div", {
                                      ...
                                      createVNode("div", { class: "py-1" }, [
                                        ...
                                        createVNode("button", {
                                          type: "button",
                                          class: "flex w-full items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed",
                                          disabled: isPostingVat.value || !canPostPph23Payable.value,
                                          onClick: ($event) => canPostPph23Payable.value && handleVatAction(postPph23Payable05)
                                        }, " Post VAT Payable PPh23 0.5% ", 8, ["disabled", "onClick"]),
                                        createVNode("button", {
                                          type: "button",
                                          class: "flex w-full items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed",
                                          disabled: isPostingVat.value || !canPostPph23Payable.value,
                                          onClick: ($event) => canPostPph23Payable.value && handleVatAction(postPph23Payable2)
                                        }, " Post VAT Payable PPh23 2% ", 8, ["disabled", "onClick"]),
                                        createVNode("div", { class: "my-1 border-t border-gray-100" }),
                                        createVNode("button", {
                                          type: "button",
                                          class: "flex w-full items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed",
                                          disabled: isPostingVat.value || !canPostVatReceivable.value,
                                          onClick: ($event) => canPostVatReceivable.value && handleVatAction(postVatReceivable11)
                                        }, " Post VAT Receivable 11% ", 8, ["disabled", "onClick"]),
                                        createVNode("button", {
                                          type: "button",
                                          class: "flex w-full items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed",
                                          disabled: isPostingVat.value || !canPostVatReceivable.value,
                                          onClick: ($event) => canPostVatReceivable.value && handleVatAction(postVatReceivable11_1)
