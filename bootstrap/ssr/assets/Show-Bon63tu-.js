import { computed, withCtx, unref, createVNode, createTextVNode, toDisplayString, createBlock, openBlock, withModifiers, createCommentVNode, withDirectives, vModelText, Fragment, renderList, useSSRContext } from "vue";
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const indexFilters = computed(() => {
      var _a;
      return {
        period_month: ((_a = props.filters) == null ? void 0 : _a.period_month) || void 0
      };
    });
    const exportPdfUrl = computed(() => {
      var _a;
      const baseUrl = route("admin-keuangan.bank-balance.export-pdf", props.bank.id);
      if (!((_a = props.filters) == null ? void 0 : _a.period_month)) {
        return baseUrl;
      }
      ...
      _push(ssrRenderComponent(AdminKeuanganLayout, _attrs, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            _push2(`<div class="py-6"${_scopeId}><div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"${_scopeId}><div class="mb-6"${_scopeId}><div class="flex items-center justify-between"${_scopeId}><div${_scopeId}><h1 class="text-2xl font-bold text-gray-900"${_scopeId}>Bank ${ssrInterpolate(__props.bank.bank_name)} Details</h1><p class="mt-1 text-sm text-gray-600"${_scopeId}> Balance and transaction details for ${ssrInterpolate(__props.bank.bank_name)}</p></div><div class="flex items-center space-x-2"${_scopeId}><a${ssrRenderAttr("href", exportPdfUrl.value)} class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-gray-600 focus:ring-offset-2" target="_blank" rel="noopener noreferrer"${_scopeId}>`);
            ...
            _push2(ssrRenderComponent(unref(Link), {
              href: _ctx.route("admin-keuangan.bank-balance.history", { bank: __props.bank.id, ...indexFilters.value }),
