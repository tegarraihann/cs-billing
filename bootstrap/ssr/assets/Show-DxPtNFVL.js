...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const typeLabel = computed(() => {
      var _a2;
      return ((_a2 = props.typeConfig) == null ? void 0 : _a2.label) || props.entry.entry_type;
    });
    const settlementForm = useForm({
      bank_account_id: "",
      settlement_date: ((_a = props.entry) == null ? void 0 : _a.payment_date) || (/* @__PURE__ */ new Date()).toISOString().slice(0, 10),
      notes: ""
    });
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          ...
          if (_push2) {
            ...
            _push2(ssrRenderComponent(unref(Link), {
              href: _ctx.route("admin-keuangan.equity.index", __props.filters || {}),
            ...
            _push2(`</div><div class="bg-white shadow rounded-lg mb-6"${_scopeId}><div class="px-6 py-6 space-y-4"${_scopeId}><div class="grid grid-cols-1 md:grid-cols-2 gap-4"${_scopeId}><div${_scopeId}><p class="text-xs text-gray-500 uppercase tracking-wide"${_scopeId}>Entry Date</p><p class="text-sm font-semibold text-gray-900"${_scopeId}>${ssrInterpolate(formatDate(__props.entry.entry_date))}</p></div>`);
            if (__props.entry.employee_name) {
              _push2(`<div${_scopeId}><p class="text-xs text-gray-500 uppercase tracking-wide"${_scopeId}>Employee Name</p><p class="text-sm font-semibold text-gray-900"${_scopeId}>${ssrInterpolate(__props.entry.employee_name)}</p></div>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`<div${_scopeId}><p class="text-xs text-gray-500 uppercase tracking-wide"${_scopeId}>Amount</p><p class="${ssrRenderClass([__props.entry.direction === "decrease" ? "text-red-600" : "text-green-600", "text-sm font-semibold"])}"${_scopeId}>${ssrInterpolate(__props.entry.direction === "decrease" ? "-" : "+")}${ssrInterpolate(formatCurrency(__props.entry.amount))}</p></div>`);
            if (__props.entry.payment_date) {
              _push2(`<div${_scopeId}><p class="text-xs text-gray-500 uppercase tracking-wide"${_scopeId}>Payment Date</p><p class="text-sm text-gray-900"${_scopeId}>${ssrInterpolate(formatDate(__props.entry.payment_date))}</p></div>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`<div${_scopeId}><p class="text-xs text-gray-500 uppercase tracking-wide"${_scopeId}>Opening Balance</p><p class="text-sm text-gray-900"${_scopeId}>${ssrInterpolate(__props.entry.is_opening ? "Yes" : "No")}</p></div><div${_scopeId}><p class="text-xs text-gray-500 uppercase tracking-wide"${_scopeId}>Status</p><span class="${ssrRenderClass([__props.entry.status === "settled" ? "bg-green-100 text-green-800" : "bg-yellow-100 text-yellow-800", "inline-flex px-2 py-1 text-xs font-semibold rounded-full"])}"${_scopeId}>${ssrInterpolate(__props.entry.status === "settled" ? "Settled" : "Recorded")}</span></div></div>`);
            ...
            if (__props.entry.affects_bank) {
              _push2(`<div class="space-y-2"${_scopeId}><div class="text-sm text-gray-700"${_scopeId}><span class="font-medium"${_scopeId}>Bank:</span> ${ssrInterpolate(((_a2 = __props.entry.bank_account) == null ? void 0 : _a2.bank_name) || "-")} (${ssrInterpolate(((_b = __props.entry.bank_account) == null ? void 0 : _b.account_number) || "-")}) </div><div class="text-sm text-gray-700"${_scopeId}><span class="font-medium"${_scopeId}>Transaction Type:</span> ${ssrInterpolate(__props.entry.bank_transaction_type || "-")}</div>`);
                  ...
                  createVNode("div", { class: "bg-white shadow rounded-lg mb-6" }, [
                    createVNode("div", { class: "px-6 py-6 space-y-4" }, [
                      createVNode("div", { class: "grid grid-cols-1 md:grid-cols-2 gap-4" }, [
                        ...
                        __props.entry.employee_name ? (openBlock(), createBlock("div", { key: 0 }, [
                          createVNode("p", { class: "text-xs text-gray-500 uppercase tracking-wide" }, "Employee Name"),
                          createVNode("p", { class: "text-sm font-semibold text-gray-900" }, toDisplayString(__props.entry.employee_name), 1)
                        ...
                        __props.entry.payment_date ? (openBlock(), createBlock("div", { key: 1 }, [
                          createVNode("p", { class: "text-xs text-gray-500 uppercase tracking-wide" }, "Payment Date"),
                          createVNode("p", { class: "text-sm text-gray-900" }, toDisplayString(formatDate(__props.entry.payment_date)), 1)
                        ])) : createCommentVNode("", true),
                        createVNode("div", null, [
