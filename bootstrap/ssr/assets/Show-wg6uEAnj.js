...
import { ArrowLeft, FileText, Percent, CreditCard } from "lucide-vue-next";
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const canPostTaxExpense = computed(() => {
      var _a, _b;
      return (((_a = props.receivable) == null ? void 0 : _a.outstanding_amount) || 0) > 0 && !((_b = props.receivable) == null ? void 0 : _b.tax_writeoff_at);
    });
      ...
      _push(ssrRenderComponent(AdminKeuanganLayout, _attrs, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          ...
          if (_push2) {
            ...
            _push2(`</span>`);
            if (canPostVat.value) {
              _push2(`<button class="inline-flex items-center justify-center px-4 py-2 min-w-[170px] bg-amber-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-wider hover:bg-amber-700 focus:bg-amber-700 active:bg-amber-900 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2 transition ease-in-out duration-150"${ssrIncludeBooleanAttr(postingVat.value) ? " disabled" : ""}${_scopeId}>`);
