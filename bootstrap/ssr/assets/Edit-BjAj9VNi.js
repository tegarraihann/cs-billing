      ...
      _push(ssrRenderComponent(AdminKeuanganLayout, _attrs, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            _push2(`<p class="mt-1 text-xs text-sage-500"${_scopeId}> Pendapatan ini akan langsung menambah saldo bank terpilih. </p></div><div${_scopeId}><label class="block text-sm font-medium text-sage-700 mb-2"${_scopeId}> Akun Pendapatan (P&amp;L) <span class="text-red-500"${_scopeId}>*</span></label><select class="${ssrRenderClass([{ "border-red-300": unref(form).errors.pl_account_id }, "w-full px-3 py-2 border border-sage-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500 text-sm"])}"${ssrIncludeBooleanAttr(__props.otherIncome.posted_to_profit_loss) ? " disabled" : ""}${_scopeId}><option value=""${ssrIncludeBooleanAttr(Array.isArray(unref(form).pl_account_id) ? ssrLooseContain(unref(form).pl_account_id, "") : ssrLooseEqual(unref(form).pl_account_id, "")) ? " selected" : ""}${_scopeId}>Pilih Akun</option><!--[-->`);
