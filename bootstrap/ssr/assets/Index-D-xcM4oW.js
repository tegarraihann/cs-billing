import { watch, mergeProps, useSSRContext, reactive, ref, computed, withCtx, unref, createVNode, toDisplayString, createBlock, createCommentVNode, withDirectives, vModelText, vModelSelect, openBlock, Fragment, renderList, createTextVNode } from "vue";
          ...
          if (_push2) {
            ...
            if (vendorSummaryRows.value.length > 0) {
              ...
              ssrRenderList(tableRows.value, (row) => {
                _push2(`<tr class="hover:bg-gray-50"${_scopeId}><td class="px-6 py-4 whitespace-nowrap"${_scopeId}><div class="text-sm font-medium text-gray-900"${_scopeId}>${ssrInterpolate(row.vendorName)}</div><div class="text-sm text-gray-500"${_scopeId}>${ssrInterpolate(row.vendorInvoiceDate ? formatDate(row.vendorInvoiceDate) : "-")}</div></td><td class="px-6 py-4"${_scopeId}><div class="text-sm font-medium text-gray-900"${_scopeId}>${ssrInterpolate(row.serviceLabel)}</div>`);
                if (row.serviceDescription) {
                  _push2(`<div class="text-sm text-gray-600"${_scopeId}>${ssrInterpolate(row.serviceDescription)}</div>`);
                } else {
                  _push2(`<!---->`);
                }
                if (row.serviceRemarks) {
                  _push2(`<div class="text-sm text-gray-500"${_scopeId}>${ssrInterpolate(row.serviceRemarks.substring(0, 50))}${ssrInterpolate(row.serviceRemarks.length > 50 ? "..." : "")}</div>`);
                } else {
                  _push2(`<!---->`);
                }
                _push2(`</td><td class="px-6 py-4 whitespace-nowrap"${_scopeId}><div class="text-sm text-gray-900"${_scopeId}>`);
                if (row.invoiceNumber) {
                  _push2(`<div${_scopeId}> Invoice: ${ssrInterpolate(row.invoiceNumber)}</div>`);
                } else {
                  _push2(`<!---->`);
                }
                if (row.salesOrder) {
                  _push2(`<div${_scopeId}> SO: ${ssrInterpolate(row.salesOrder.order_number)}</div>`);
                } else {
                  _push2(`<!---->`);
                }
                _push2(`</div></td><td class="px-6 py-4 whitespace-nowrap text-right text-sm text-gray-900"${_scopeId}> Rp ${ssrInterpolate(formatNumber(row.amount))}</td><td class="px-6 py-4 whitespace-nowrap text-right text-sm text-gray-900"${_scopeId}> Rp ${ssrInterpolate(formatNumber(row.paidAmount))}</td><td class="px-6 py-4 whitespace-nowrap text-right text-sm text-gray-900"${_scopeId}> Rp ${ssrInterpolate(formatNumber(row.outstanding))}</td><td class="px-6 py-4 whitespace-nowrap text-center"${_scopeId}><span class="${ssrRenderClass([getStatusClass(row.status), "inline-flex px-2 py-1 text-xs font-semibold rounded-full"])}"${_scopeId}>${ssrInterpolate(getStatusText(row.status))} `);
                if (row.daysOverdue > 0) {
                  _push2(`<span class="ml-1"${_scopeId}> (${ssrInterpolate(row.daysOverdue)} hari) </span>`);
                } else {
                  _push2(`<!---->`);
                }
            ...
            _push2(ssrRenderComponent(_sfc_main$1, {
              ...
              summary: withCtx((_2, _push3, _parent3, _scopeId2) => {
                if (_push3) {
                  _push3(`<div class="mb-4 bg-gray-50 p-3 rounded-md"${_scopeId2}><p class="text-sm text-gray-600"${_scopeId2}>Vendor: ${ssrInterpolate(modalVendorName.value)}</p><p class="text-sm text-gray-600"${_scopeId2}>Komponen: ${ssrInterpolate(modalServiceLabel.value)}</p><p class="text-sm text-gray-600"${_scopeId2}>Outstanding: Rp ${ssrInterpolate(formatNumber(modalOutstanding.value))}</p></div>`);
                } else {
                  return [
