import { computed, ref, reactive, onMounted, onBeforeUnmount, watch, withCtx, unref, createVNode, createBlock, createCommentVNode, toDisplayString, createTextVNode, openBlock, Fragment, renderList, withModifiers, withDirectives, vModelSelect, vModelText, useSSRContext } from "vue";
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const displayPaymentNotes = computed(() => {
      var _a;
      return props.paymentNotes || ((_a = props.payable) == null ? void 0 : _a.payment_notes) || "";
    });
    ...
    const payableStatus = computed(() => {
      var _a;
      return summaryStatus.value ?? ((_a = payable.value) == null ? void 0 : _a.status) ?? "unpaid";
    });
    const payableOutstanding = computed(() => {
      var _a;
      return Number(summary.value.total_outstanding ?? ((_a = payable.value) == null ? void 0 : _a.outstanding_amount) ?? 0);
    });
    ...
    const canPostVatPayable = computed(() => {
      var _a;
      return payableOutstanding.value > 0 && payableStatus.value !== "paid" && !((_a = payable.value) == null ? void 0 : _a.vat_payable_posted_at);
    });
    const canPostPph23Payable = computed(() => payableOutstanding.value > 0 && payableStatus.value !== "paid");
    const canPostPph23PayableFor = (component) => canPostPph23Payable.value && Number((component == null ? void 0 : component.outstanding_amount) ?? 0) > 0;
    const canPostVatReceivableFor = (component) => (component == null ? void 0 : component.component_type) === "vat_reimbursement" && Number((component == null ? void 0 : component.outstanding_amount) ?? 0) <= 0 && (component == null ? void 0 : component.status) === "paid" && !(component == null ? void 0 : component.vat_receivable_posted_at);
    const activeVatMenuId = ref(null);
    const isPostingVat = computed(() => postingVat11.value || postingVat11_1.value || postingPph23_05.value || postingPph23_2.value || postingVatReceivable11.value || postingVatReceivable11_1.value);
    ...
    const headerSubtitle = computed(() => {
      ...
      if ((_b = payable.value) == null ? void 0 : _b.is_opening) {
      ...
      if ((_c = summary.value.vendor_names) == null ? void 0 : _c.length) {
    ...
    const getComponentVendorInvoice = (component) => {
      var _a;
      const invoiceNumber = (_a = component == null ? void 0 : component.related_items) == null ? void 0 : _a.vendor_invoice_number;
    ...
    const postVatReceivable11 = (component = null) => {
      ...
      const targetPayableId = (component == null ? void 0 : component.parent_payable_id) || (component == null ? void 0 : component.account_payable_id) || payable.value.id;
      openConfirm(
        ...
        () => {
          ...
          router.post(
            ...
            {
              component_id: (component == null ? void 0 : component.id) ?? null
            },
            {
              onFinish: () => {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(ssrRenderComponent(AdminKeuanganLayout, _attrs, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          ...
          if (_push2) {
            _push2(ssrRenderComponent(unref(Head), { title: "Payable Details" }, null, _parent2, _scopeId));
            ...
            _push2(`<div class="py-6"${_scopeId}><div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"${_scopeId}><div class="flex justify-between items-center mb-6"${_scopeId}><div class="flex flex-wrap items-center justify-end gap-2"${_scopeId}><button class="text-gray-400 hover:text-gray-600"${_scopeId}>`);
            _push2(ssrRenderComponent(unref(ArrowLeft), { class: "w-6 h-6" }, null, _parent2, _scopeId));
            _push2(`</button><div${_scopeId}><h1 class="text-2xl font-bold text-gray-900"${_scopeId}>Payable Details</h1><p class="mt-1 text-sm text-gray-600"${_scopeId}>${ssrInterpolate(headerSubtitle.value)}</p></div></div><div class="flex items-center space-x-3"${_scopeId}><span class="${ssrRenderClass([getStatusClass(summaryStatus.value), "inline-flex px-3 py-1 text-sm font-semibold rounded-full"])}"${_scopeId}>${ssrInterpolate(getStatusText(summaryStatus.value))} `);
            if (overdueDays.value > 0) {
              _push2(`<span class="ml-1"${_scopeId}> (${ssrInterpolate(overdueDays.value)} days overdue) </span>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`</span>`);
            ...
            if (payable.value.is_opening) {
              _push2(`<div${_scopeId}><label class="text-sm font-medium text-gray-500"${_scopeId}>Opening Type</label><p class="text-sm text-gray-900"${_scopeId}>${ssrInterpolate(openingTypeLabel(payable.value.opening_type))}</p></div>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`</div></div><div${_scopeId}><div class="space-y-3"${_scopeId}>`);
            ...
            if (payable.value.is_opening && payable.value.source_so_number) {
              _push2(`<div${_scopeId}><label class="text-sm font-medium text-gray-500"${_scopeId}>Source SO Number</label><p class="text-sm text-gray-900"${_scopeId}>${ssrInterpolate(payable.value.source_so_number)}</p></div>`);
            } else {
              _push2(`<!---->`);
            }
            ...
            if (payable.value.is_opening && payable.value.opening_payment_date) {
              _push2(`<div${_scopeId}><label class="text-sm font-medium text-gray-500"${_scopeId}>Opening Payment Date</label><p class="text-sm text-gray-900"${_scopeId}>${ssrInterpolate(formatDate(payable.value.opening_payment_date))}</p></div>`);
            } else {
              _push2(`<!---->`);
            }
            ...
            if (visibleComponents.value.length) {
              _push2(`<div class="bg-white rounded-lg shadow-sm p-6 mb-6"${_scopeId}><div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4"${_scopeId}><div${_scopeId}><h2 class="text-lg font-semibold text-gray-900"${_scopeId}>Payable Component Breakdown</h2><p class="text-sm text-gray-500"${_scopeId}>All cost components (main invoice, reimbursement, operational) are listed in this table.</p></div><button class="inline-flex items-center px-4 py-2 border border-red-200 text-red-700 text-sm font-medium rounded-md bg-red-50 hover:bg-red-100 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition"${_scopeId}>`);
              _push2(ssrRenderComponent(unref(Plus), { class: "w-4 h-4 mr-2" }, null, _parent2, _scopeId));
              _push2(` Add Cost </button></div><div class="overflow-x-auto"${_scopeId}><table class="min-w-full divide-y divide-gray-200"${_scopeId}><thead class="bg-gray-50"${_scopeId}><tr${_scopeId}><th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"${_scopeId}> Component </th><th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"${_scopeId}> Category </th><th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"${_scopeId}> Recipient </th><th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"${_scopeId}> Vendor Invoice </th><th class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider"${_scopeId}> Payable Amount </th><th class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider"${_scopeId}> Paid </th><th class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider"${_scopeId}> Outstanding </th><th class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider"${_scopeId}> Status </th><th class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider"${_scopeId}> Actions </th></tr></thead><tbody class="bg-white divide-y divide-gray-200"${_scopeId}><!--[-->`);
              ssrRenderList(visibleComponents.value, (component) => {
                _push2(`<tr${_scopeId}><td class="px-4 py-3 text-sm text-gray-900"${_scopeId}><div class="font-medium text-gray-900"${_scopeId}>${ssrInterpolate(component.description || "No description")}</div></td><td class="px-4 py-3 text-sm text-gray-900"${_scopeId}>${ssrInterpolate(getComponentCategory(component) || "-")}</td><td class="px-4 py-3 text-sm text-gray-900"${_scopeId}>${ssrInterpolate(component.recipient_name || "-")}</td><td class="px-4 py-3 text-sm text-gray-900"${_scopeId}>${ssrInterpolate(getComponentVendorInvoice(component))}</td><td class="px-4 py-3 text-sm text-gray-900 text-right"${_scopeId}> Rp ${ssrInterpolate(formatNumber(component.amount))}</td><td class="px-4 py-3 text-sm text-gray-900 text-right"${_scopeId}> Rp ${ssrInterpolate(formatNumber(component.paid_amount))}</td><td class="px-4 py-3 text-sm text-gray-900 text-right"${_scopeId}> Rp ${ssrInterpolate(formatNumber(component.outstanding_amount))}</td><td class="px-4 py-3 text-sm text-right"${_scopeId}><span class="${ssrRenderClass([getStatusClass(component.status), "inline-flex px-2 py-1 text-xs font-semibold rounded-full"])}"${_scopeId}>${ssrInterpolate(getStatusText(component.status))}</span></td><td class="px-4 py-3 text-sm text-right"${_scopeId}>`);
                if (hasVatActions()) {
                  _push2(`<div class="relative inline-flex justify-end w-full"${_scopeId}><button type="button" class="inline-flex items-center justify-center rounded-md border border-sage-300 bg-white px-2.5 py-2 text-sage-700 hover:bg-sage-50 focus:outline-none focus:ring-2 focus:ring-sage-500 focus:ring-offset-2 transition" aria-label="VAT actions"${_scopeId}>`);
                  _push2(ssrRenderComponent(unref(MoreVertical), { class: "h-4 w-4" }, null, _parent2, _scopeId));
                  _push2(`</button>`);
                  if (isVatMenuOpen(component.id)) {
                    _push2(`<div class="absolute right-0 z-20 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5"${_scopeId}><div class="py-1"${_scopeId}><button type="button" class="flex w-full items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed"${ssrIncludeBooleanAttr(isPostingVat.value || !canPostVatPayable.value) ? " disabled" : ""}${_scopeId}> Post VAT Payable 11% </button><button type="button" class="flex w-full items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed"${ssrIncludeBooleanAttr(isPostingVat.value || !canPostVatPayable.value) ? " disabled" : ""}${_scopeId}> Post VAT Payable 1.1% </button><button type="button" class="flex w-full items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed"${ssrIncludeBooleanAttr(isPostingVat.value || !canPostPph23PayableFor(component)) ? " disabled" : ""}${_scopeId}> Post VAT Payable PPh23 0.5% </button><button type="button" class="flex w-full items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed"${ssrIncludeBooleanAttr(isPostingVat.value || !canPostPph23PayableFor(component)) ? " disabled" : ""}${_scopeId}> Post VAT Payable PPh23 2% </button><div class="my-1 border-t border-gray-100"${_scopeId}></div><button type="button" class="flex w-full items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed"${ssrIncludeBooleanAttr(isPostingVat.value || !canPostVatReceivableFor(component)) ? " disabled" : ""}${_scopeId}> Post VAT Receivable 11% </button><button type="button" class="flex w-full items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed"${ssrIncludeBooleanAttr(isPostingVat.value || !canPostVatReceivableFor(component)) ? " disabled" : ""}${_scopeId}> Post VAT Receivable 1.1% </button></div></div>`);
                  } else {
                    _push2(`<!---->`);
                  }
                  _push2(`</div>`);
            ...
            if (payable.value.payment_method || displayPaymentNotes.value) {
              ...
              _push2(`</div>`);
              if (displayPaymentNotes.value) {
                _push2(`<div class="mt-4"${_scopeId}><label class="text-sm font-medium text-gray-500"${_scopeId}>Payment Notes</label><div class="mt-1 bg-gray-50 p-3 rounded-md"${_scopeId}><p class="text-sm text-gray-700 whitespace-pre-line"${_scopeId}>${ssrInterpolate(displayPaymentNotes.value)}</p></div></div>`);
              } else {
                _push2(`<!---->`);
              }
              _push2(`</div>`);
            ...
            if (showPaymentModal.value) {
              ...
              if (hasMultipleComponents.value) {
                _push2(`<div class="mb-4"${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-1"${_scopeId}>Payment Component *</label><select required class="${ssrRenderClass([unref(paymentForm).errors.component_id ? "border-red-300 focus:border-red-500 focus:ring-red-500" : "border-gray-300", "w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"])}"${_scopeId}><option value=""${ssrIncludeBooleanAttr(Array.isArray(unref(paymentForm).component_id) ? ssrLooseContain(unref(paymentForm).component_id, "") : ssrLooseEqual(unref(paymentForm).component_id, "")) ? " selected" : ""}${_scopeId}>Select Component</option><!--[-->`);
              ...
              _push2(`<div class="mb-4"${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-1"${_scopeId}>Payment Method *</label><select required class="${ssrRenderClass([unref(paymentForm).errors.payment_method ? "border-red-300 focus:border-red-500 focus:ring-red-500" : "border-gray-300", "w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"])}"${_scopeId}><option value=""${ssrIncludeBooleanAttr(Array.isArray(unref(paymentForm).payment_method) ? ssrLooseContain(unref(paymentForm).payment_method, "") : ssrLooseEqual(unref(paymentForm).payment_method, "")) ? " selected" : ""}${_scopeId}>Select Payment Method</option><option value="Transfer Bank"${ssrIncludeBooleanAttr(Array.isArray(unref(paymentForm).payment_method) ? ssrLooseContain(unref(paymentForm).payment_method, "Transfer Bank") : ssrLooseEqual(unref(paymentForm).payment_method, "Transfer Bank")) ? " selected" : ""}${_scopeId}>Bank Transfer</option><option value="Cash"${ssrIncludeBooleanAttr(Array.isArray(unref(paymentForm).payment_method) ? ssrLooseContain(unref(paymentForm).payment_method, "Cash") : ssrLooseEqual(unref(paymentForm).payment_method, "Cash")) ? " selected" : ""}${_scopeId}>Cash</option><option value="Check"${ssrIncludeBooleanAttr(Array.isArray(unref(paymentForm).payment_method) ? ssrLooseContain(unref(paymentForm).payment_method, "Check") : ssrLooseEqual(unref(paymentForm).payment_method, "Check")) ? " selected" : ""}${_scopeId}>Check</option><option value="Other"${ssrIncludeBooleanAttr(Array.isArray(unref(paymentForm).payment_method) ? ssrLooseContain(unref(paymentForm).payment_method, "Other") : ssrLooseEqual(unref(paymentForm).payment_method, "Other")) ? " selected" : ""}${_scopeId}>Other</option></select>`);
            ...
            if (showAdditionalCostModal.value) {
              _push2(`<div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50"${_scopeId}><div class="relative top-20 mx-auto p-5 border w-[420px] shadow-lg rounded-md bg-white"${_scopeId}><div class="flex items-center justify-between mb-4"${_scopeId}><h3 class="text-lg font-medium text-gray-900"${_scopeId}>Add Cost</h3><button class="text-gray-400 hover:text-gray-600"${_scopeId}>×</button></div><form${_scopeId}><div class="mb-4"${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-1"${_scopeId}>Cost Type *</label><select class="w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500"${_scopeId}><option value="operational_cost"${ssrIncludeBooleanAttr(Array.isArray(unref(additionalCostForm).component_type) ? ssrLooseContain(unref(additionalCostForm).component_type, "operational_cost") : ssrLooseEqual(unref(additionalCostForm).component_type, "operational_cost")) ? " selected" : ""}${_scopeId}>Operational Cost (Internal)</option><option value="reimbursement"${ssrIncludeBooleanAttr(Array.isArray(unref(additionalCostForm).component_type) ? ssrLooseContain(unref(additionalCostForm).component_type, "reimbursement") : ssrLooseEqual(unref(additionalCostForm).component_type, "reimbursement")) ? " selected" : ""}${_scopeId}>Reimbursement (Billable)</option><option value="vat_reimbursement"${ssrIncludeBooleanAttr(Array.isArray(unref(additionalCostForm).component_type) ? ssrLooseContain(unref(additionalCostForm).component_type, "vat_reimbursement") : ssrLooseEqual(unref(additionalCostForm).component_type, "vat_reimbursement")) ? " selected" : ""}${_scopeId}>VAT Reimbursement Vendor</option></select><p class="text-xs text-gray-500 mt-1"${_scopeId}> Operational costs only affect profit. Reimbursement will automatically go to the reimbursement invoice. VAT reimbursement does not go to the customer invoice and will be posted to Financial Position after the payable is paid. </p></div>`);
              if (shouldShowVatRateField.value) {
                _push2(`<div class="mb-4"${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-1"${_scopeId}>VAT Rate *</label><select class="w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500"${ssrIncludeBooleanAttr(shouldShowVatRateField.value) ? " required" : ""}${_scopeId}><option value=""${ssrIncludeBooleanAttr(Array.isArray(unref(additionalCostForm).vat_rate) ? ssrLooseContain(unref(additionalCostForm).vat_rate, "") : ssrLooseEqual(unref(additionalCostForm).vat_rate, "")) ? " selected" : ""}${_scopeId}>-- Select VAT Rate --</option><option value="11"${ssrIncludeBooleanAttr(Array.isArray(unref(additionalCostForm).vat_rate) ? ssrLooseContain(unref(additionalCostForm).vat_rate, "11") : ssrLooseEqual(unref(additionalCostForm).vat_rate, "11")) ? " selected" : ""}${_scopeId}>11%</option><option value="1.1"${ssrIncludeBooleanAttr(Array.isArray(unref(additionalCostForm).vat_rate) ? ssrLooseContain(unref(additionalCostForm).vat_rate, "1.1") : ssrLooseEqual(unref(additionalCostForm).vat_rate, "1.1")) ? " selected" : ""}${_scopeId}>1.1%</option></select>`);
                if (unref(additionalCostForm).errors.vat_rate) {
                  _push2(`<p class="text-sm text-red-600 mt-1"${_scopeId}>${ssrInterpolate(unref(additionalCostForm).errors.vat_rate)}</p>`);
                } else {
                  _push2(`<!---->`);
                }
                _push2(`</div>`);
              ...
              _push2(`<div class="mb-4"${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-1"${_scopeId}>Description *</label><input${ssrRenderAttr("value", unref(additionalCostForm).description)} type="text" class="w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500" placeholder="Example: Bank Admin Fee" required${_scopeId}>`);
              ...
              if (shouldShowCategoryField.value) {
                _push2(`<div class="mb-4"${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-1"${_scopeId}>Cost Category *</label>`);
                _push2(ssrRenderComponent(_sfc_main$1, {
                  modelValue: unref(additionalCostForm).category_id,
                ...
                _push2(`<div class="mb-4"${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-1"${_scopeId}>Cost Category</label><div class="px-3 py-2 rounded-md border border-gray-200 bg-gray-50 text-gray-700"${_scopeId}>${ssrInterpolate(getCategoryNameById(unref(additionalCostForm).category_id) || "Matches component")}</div><p class="text-xs text-gray-500 mt-1"${_scopeId}>Category follows the component being viewed.</p></div>`);
              } else {
                _push2(`<!---->`);
              }
              _push2(`<div class="mb-4"${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-1"${_scopeId}>Vendor / Recipient</label>`);
              _push2(ssrRenderComponent(_sfc_main$1, {
                modelValue: unref(additionalCostForm).vendor_id,
              ...
              _push2(`</div><div class="mb-4"${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-1"${_scopeId}>Notes</label><textarea rows="3" class="w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500" placeholder="Additional notes"${_scopeId}>${ssrInterpolate(unref(additionalCostForm).notes)}</textarea>`);
              ...
              _push2(`</div><div class="flex justify-end space-x-3"${_scopeId}><button type="button" class="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50"${_scopeId}> Cancel </button><button type="submit"${ssrIncludeBooleanAttr(unref(additionalCostForm).processing) ? " disabled" : ""} class="px-4 py-2 bg-red-600 text-white rounded-md text-sm font-medium hover:bg-red-700 disabled:opacity-50"${_scopeId}>${ssrInterpolate(unref(additionalCostForm).processing ? "Saving..." : "Save Cost")}</button></div></form></div></div>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`</div></div>`);
            ...
            return [
              createVNode(unref(Head), { title: "Payable Details" }),
                ...
                createVNode("div", { class: "max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" }, [
                  createVNode("div", { class: "flex justify-between items-center mb-6" }, [
                    createVNode("div", { class: "flex flex-wrap items-center justify-end gap-2" }, [
                      ...
                      createVNode("div", null, [
                        createVNode("h1", { class: "text-2xl font-bold text-gray-900" }, "Payable Details"),
                    ...
                    createVNode("div", { class: "flex items-center space-x-3" }, [
                      createVNode("span", {
                        ...
                        overdueDays.value > 0 ? (openBlock(), createBlock("span", {
                          key: 0,
                          class: "ml-1"
                        }, " (" + toDisplayString(overdueDays.value) + " days overdue) ", 1)) : createCommentVNode("", true)
                      ], 2),
                  ...
                  createVNode("div", { class: "bg-white shadow overflow-hidden sm:rounded-md mb-6" }, [
                    createVNode("div", { class: "px-4 py-5 sm:p-6" }, [
                      createVNode("h3", { class: "text-lg leading-6 font-medium text-gray-900 mb-4" }, "Vendor Information"),
                    ...
                    createVNode("div", { class: "bg-white rounded-lg shadow-sm p-6 mb-6" }, [
                      createVNode("h2", { class: "text-lg font-semibold text-gray-900 mb-4" }, "Service Information"),
                      createVNode("div", { class: "grid grid-cols-1 md:grid-cols-2 gap-6" }, [
                        createVNode("div", null, [
                          createVNode("div", { class: "space-y-3" }, [
                            createVNode("div", null, [
                            ...
                            payable.value.is_opening ? (openBlock(), createBlock("div", { key: 2 }, [
                              createVNode("label", { class: "text-sm font-medium text-gray-500" }, "Opening Balance"),
                              createVNode("p", { class: "text-sm text-gray-900" }, "Yes")
                            ])) : createCommentVNode("", true),
                            payable.value.is_opening ? (openBlock(), createBlock("div", { key: 3 }, [
                              createVNode("label", { class: "text-sm font-medium text-gray-500" }, "Opening Type"),
                              createVNode("p", { class: "text-sm text-gray-900" }, toDisplayString(openingTypeLabel(payable.value.opening_type)), 1)
                            ])) : createCommentVNode("", true)
                          ])
                        ]),
                        createVNode("div", null, [
                          createVNode("div", { class: "space-y-3" }, [
                            ...
                            payable.value.is_opening && payable.value.source_so_number ? (openBlock(), createBlock("div", { key: 1 }, [
                              createVNode("label", { class: "text-sm font-medium text-gray-500" }, "Source SO Number"),
                              createVNode("p", { class: "text-sm text-gray-900" }, toDisplayString(payable.value.source_so_number), 1)
                            ])) : createCommentVNode("", true),
                            payable.value.payment_due_date ? (openBlock(), createBlock("div", { key: 2 }, [
                            ...
                            payable.value.is_opening && payable.value.opening_payment_date ? (openBlock(), createBlock("div", { key: 3 }, [
                              createVNode("label", { class: "text-sm font-medium text-gray-500" }, "Opening Payment Date"),
                              createVNode("p", { class: "text-sm text-gray-900" }, toDisplayString(formatDate(payable.value.opening_payment_date)), 1)
                            ])) : createCommentVNode("", true),
                            payable.value.payment_date ? (openBlock(), createBlock("div", { key: 4 }, [
                    ...
                    visibleComponents.value.length ? (openBlock(), createBlock("div", {
                      ...
                      createVNode("div", { class: "flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4" }, [
                        createVNode("div", null, [
                          createVNode("h2", { class: "text-lg font-semibold text-gray-900" }, "Payable Component Breakdown"),
                          createVNode("p", { class: "text-sm text-gray-500" }, "All cost components (main invoice, reimbursement, operational) are listed in this table.")
                        ]),
                        createVNode("button", {
                        ...
                        createVNode("table", { class: "min-w-full divide-y divide-gray-200" }, [
                          createVNode("thead", { class: "bg-gray-50" }, [
                            createVNode("tr", null, [
                              ...
                              createVNode("th", { class: "px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider" }, " Category "),
                              createVNode("th", { class: "px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider" }, " Recipient "),
                              createVNode("th", { class: "px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider" }, " Vendor Invoice "),
                              createVNode("th", { class: "px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider" }, " Payable Amount "),
                            ...
                            (openBlock(true), createBlock(Fragment, null, renderList(visibleComponents.value, (component) => {
                              return openBlock(), createBlock("tr", {
                                ...
                                createVNode("td", { class: "px-4 py-3 text-sm text-gray-900" }, [
                                  createVNode("div", { class: "font-medium text-gray-900" }, toDisplayString(component.description || "No description"), 1)
                                ]),
                                ...
                                createVNode("td", { class: "px-4 py-3 text-sm text-gray-900" }, toDisplayString(getComponentVendorInvoice(component)), 1),
                                ...
                                createVNode("td", { class: "px-4 py-3 text-sm text-right" }, [
                                  hasVatActions() ? (openBlock(), createBlock("div", {
                                    key: 0,
                                    ...
                                    createVNode("button", {
                                      ...
                                      onClick: withModifiers(($event) => toggleVatMenu(component.id), ["stop"]),
                                      "aria-label": "VAT actions"
                                    }, [
                                      createVNode(unref(MoreVertical), { class: "h-4 w-4" })
                                    ], 8, ["onClick"]),
                                    isVatMenuOpen(component.id) ? (openBlock(), createBlock("div", {
                                      key: 0,
                                      ...
                                      createVNode("div", { class: "py-1" }, [
                                        createVNode("button", {
                                          ...
                                          disabled: isPostingVat.value || !canPostVatPayable.value,
                                          onClick: ($event) => canPostVatPayable.value && handleVatAction(postVatPayable11)
                                        }, " Post VAT Payable 11% ", 8, ["disabled", "onClick"]),
                                        createVNode("button", {
                                          type: "button",
                                          class: "flex w-full items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed",
                                          disabled: isPostingVat.value || !canPostVatPayable.value,
                                          onClick: ($event) => canPostVatPayable.value && handleVatAction(postVatPayable11_1)
                                        }, " Post VAT Payable 1.1% ", 8, ["disabled", "onClick"]),
                                        createVNode("button", {
                                          type: "button",
                                          class: "flex w-full items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed",
                                          disabled: isPostingVat.value || !canPostPph23PayableFor(component),
                                          onClick: ($event) => canPostPph23PayableFor(component) && handleVatAction(postPph23Payable05, component)
                                        }, " Post VAT Payable PPh23 0.5% ", 8, ["disabled", "onClick"]),
                                        createVNode("button", {
                                          type: "button",
                                          class: "flex w-full items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed",
                                          disabled: isPostingVat.value || !canPostPph23PayableFor(component),
                                          onClick: ($event) => canPostPph23PayableFor(component) && handleVatAction(postPph23Payable2, component)
                                        }, " Post VAT Payable PPh23 2% ", 8, ["disabled", "onClick"]),
                                        createVNode("div", { class: "my-1 border-t border-gray-100" }),
                                        createVNode("button", {
                                          type: "button",
                                          class: "flex w-full items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed",
                                          disabled: isPostingVat.value || !canPostVatReceivableFor(component),
                                          onClick: ($event) => canPostVatReceivableFor(component) && handleVatAction(postVatReceivable11, component)
                                        }, " Post VAT Receivable 11% ", 8, ["disabled", "onClick"]),
                                        createVNode("button", {
                                          type: "button",
                                          class: "flex w-full items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed",
                                          disabled: isPostingVat.value || !canPostVatReceivableFor(component),
                                          onClick: ($event) => canPostVatReceivableFor(component) && handleVatAction(postVatReceivable11_1, component)
                                        }, " Post VAT Receivable 1.1% ", 8, ["disabled", "onClick"])
                                      ])
                    ...
                    payable.value.payment_method || displayPaymentNotes.value ? (openBlock(), createBlock("div", {
                      key: 1,
                      class: "bg-white rounded-lg shadow-sm p-6 mb-6"
                    }, [
                      ...
                      displayPaymentNotes.value ? (openBlock(), createBlock("div", {
                        key: 0,
                        class: "mt-4"
                      }, [
                        createVNode("label", { class: "text-sm font-medium text-gray-500" }, "Payment Notes"),
                        createVNode("div", { class: "mt-1 bg-gray-50 p-3 rounded-md" }, [
                          createVNode("p", { class: "text-sm text-gray-700 whitespace-pre-line" }, toDisplayString(displayPaymentNotes.value), 1)
                        ])
                      ])) : createCommentVNode("", true)
                    ])) : createCommentVNode("", true),
                    ...
                    createVNode("div", { class: "relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white" }, [
                      createVNode("div", { class: "mt-3" }, [
                        ...
                        createVNode("form", {
                          ...
                          createVNode("div", { class: "mb-4" }, [
                            createVNode("label", { class: "block text-sm font-medium text-gray-700 mb-1" }, "Payment Source *"),
                            withDirectives(createVNode("select", {
                          ...
                          createVNode("div", { class: "mb-4" }, [
                            ...
                            withDirectives(createVNode("select", {
                              ...
                              createVNode("option", { value: "Transfer Bank" }, "Bank Transfer"),
                  ...
                  showAdditionalCostModal.value ? (openBlock(), createBlock("div", {
                    ...
                    createVNode("div", { class: "relative top-20 mx-auto p-5 border w-[420px] shadow-lg rounded-md bg-white" }, [
                      createVNode("div", { class: "flex items-center justify-between mb-4" }, [
                        createVNode("h3", { class: "text-lg font-medium text-gray-900" }, "Add Cost"),
                        createVNode("button", {
                      ...
                      createVNode("form", {
                        ...
                        createVNode("div", { class: "mb-4" }, [
                          createVNode("label", { class: "block text-sm font-medium text-gray-700 mb-1" }, "Cost Type *"),
                          withDirectives(createVNode("select", {
                            "onUpdate:modelValue": ($event) => unref(additionalCostForm).component_type = $event,
                            class: "w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500"
                          }, [
                            createVNode("option", { value: "operational_cost" }, "Operational Cost (Internal)"),
                            createVNode("option", { value: "reimbursement" }, "Reimbursement (Billable)"),
                            createVNode("option", { value: "vat_reimbursement" }, "VAT Reimbursement Vendor")
                          ], 8, ["onUpdate:modelValue"]), [
                            [vModelSelect, unref(additionalCostForm).component_type]
                          ]),
                          createVNode("p", { class: "text-xs text-gray-500 mt-1" }, " Operational costs only affect profit. Reimbursement will automatically go to the reimbursement invoice. VAT reimbursement does not go to the customer invoice and will be posted to Financial Position after the payable is paid. ")
                        ]),
                        shouldShowVatRateField.value ? (openBlock(), createBlock("div", {
                          key: 0,
                          class: "mb-4"
                        }, [
                          createVNode("label", { class: "block text-sm font-medium text-gray-700 mb-1" }, "VAT Rate *"),
                          withDirectives(createVNode("select", {
                            "onUpdate:modelValue": ($event) => unref(additionalCostForm).vat_rate = $event,
                            class: "w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500",
                            required: shouldShowVatRateField.value
                          }, [
                            createVNode("option", { value: "" }, "-- Select VAT Rate --"),
                            createVNode("option", { value: "11" }, "11%"),
                            createVNode("option", { value: "1.1" }, "1.1%")
                          ], 8, ["onUpdate:modelValue", "required"]), [
                            [vModelSelect, unref(additionalCostForm).vat_rate]
                          ]),
                          unref(additionalCostForm).errors.vat_rate ? (openBlock(), createBlock("p", {
                            key: 0,
                            class: "text-sm text-red-600 mt-1"
                          }, toDisplayString(unref(additionalCostForm).errors.vat_rate), 1)) : createCommentVNode("", true)
                        ])) : createCommentVNode("", true),
                        createVNode("div", { class: "mb-4" }, [
                          ...
                          withDirectives(createVNode("input", {
                            ...
                            type: "text",
                            class: "w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500",
                            placeholder: "Example: Bank Admin Fee",
                            required: ""
                          }, null, 8, ["onUpdate:modelValue"]), [
                        ...
                        shouldShowCategoryField.value ? (openBlock(), createBlock("div", {
                          ...
                          createVNode("label", { class: "block text-sm font-medium text-gray-700 mb-1" }, "Cost Category *"),
                          ...
                          createVNode("label", { class: "block text-sm font-medium text-gray-700 mb-1" }, "Cost Category"),
                          createVNode("div", { class: "px-3 py-2 rounded-md border border-gray-200 bg-gray-50 text-gray-700" }, toDisplayString(getCategoryNameById(unref(additionalCostForm).category_id) || "Matches component"), 1),
                          createVNode("p", { class: "text-xs text-gray-500 mt-1" }, "Category follows the component being viewed.")
                        ])) : createCommentVNode("", true),
                        createVNode("div", { class: "mb-4" }, [
                        ...
                        createVNode("div", { class: "flex justify-end space-x-3" }, [
                          ...
                          }, toDisplayString(unref(additionalCostForm).processing ? "Saving..." : "Save Cost"), 9, ["disabled"])
                        ])
                      ], 32)
                    ])
                  ])) : createCommentVNode("", true)
