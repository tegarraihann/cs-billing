import { useSSRContext, ref, onMounted, computed, reactive, watch, withCtx, unref, createBlock, createTextVNode, openBlock, createVNode, withModifiers, createCommentVNode, withDirectives, Fragment, renderList, toDisplayString, vModelSelect, vModelText } from "vue";
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const getPreInvoiceComponent = (salesOrder, componentType) => {
      var _a;
      const components = ((_a = salesOrder == null ? void 0 : salesOrder.pre_invoice_receivable) == null ? void 0 : _a.components) ?? [];
    ...
    const populateItemsFromSalesOrder = (salesOrder, overrides = {}) => {
      ...
      const reimbursementSource = (overrides.reimbursement_items ?? salesOrder.reimbursement_items ?? []).filter((item) => {
        const customerStatus = String((item == null ? void 0 : item.customer_payment_status) ?? "").trim().toLowerCase();
        ...
        const customerOutstandingRaw = item == null ? void 0 : item.customer_outstanding_amount;
        if (customerOutstandingRaw !== void 0 && customerOutstandingRaw !== null && customerOutstandingRaw !== "") {
          return normalizeNumber(customerOutstandingRaw) > 0.01;
        }
        const quantity = resolveQuantityValue((item == null ? void 0 : item.quantity) ?? (item == null ? void 0 : item.qty) ?? 1);
        const lineTotal = normalizeNumber(item == null ? void 0 : item.amount) * quantity;
        const customerPaid = normalizeNumber(item == null ? void 0 : item.customer_paid_amount);
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(ssrRenderComponent(AdminKeuanganLayout, _attrs, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<div class="p-4 sm:p-6 lg:p-8" data-v-991a22e9${_scopeId}><div class="bg-white rounded-lg shadow-sm p-6 mb-6 border border-sage-200" data-v-991a22e9${_scopeId}><div class="flex items-center justify-between" data-v-991a22e9${_scopeId}><div data-v-991a22e9${_scopeId}><h2 class="text-2xl font-bold text-sage-800" data-v-991a22e9${_scopeId}>Create New Invoice</h2><p class="text-sage-600" data-v-991a22e9${_scopeId}>Create an invoice from an approved sales order.</p></div>`);
            _push2(ssrRenderComponent(unref(Link), {
              ...
              default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                if (_push3) {
                  _push3(`<svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" data-v-991a22e9${_scopeId2}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" data-v-991a22e9${_scopeId2}></path></svg> Back `);
                } else {
                  return [
            ...
            _push2(`</div></div><form class="space-y-6" data-v-991a22e9${_scopeId}><div class="bg-white rounded-lg shadow-sm p-6 border border-sage-200" data-v-991a22e9${_scopeId}><h3 class="text-lg font-semibold text-sage-800 mb-4" data-v-991a22e9${_scopeId}>Select Sales Order &amp; Invoice Type</h3><div class="grid grid-cols-1 md:grid-cols-2 gap-6" data-v-991a22e9${_scopeId}><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>Sales Order</label><select class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500" required data-v-991a22e9${_scopeId}><option value="" data-v-991a22e9${ssrIncludeBooleanAttr(Array.isArray(unref(form).sales_order_id) ? ssrLooseContain(unref(form).sales_order_id, "") : ssrLooseEqual(unref(form).sales_order_id, "")) ? " selected" : ""}${_scopeId}>Select Sales Order</option><!--[-->`);
            ssrRenderList(__props.salesOrders, (order) => {
              var _a;
              _push2(`<option${ssrRenderAttr("value", order.id)} data-v-991a22e9${ssrIncludeBooleanAttr(Array.isArray(unref(form).sales_order_id) ? ssrLooseContain(unref(form).sales_order_id, order.id) : ssrLooseEqual(unref(form).sales_order_id, order.id)) ? " selected" : ""}${_scopeId}>${ssrInterpolate(order.order_number)} - ${ssrInterpolate(order.customer || order.customer_name || "No Customer")} (${ssrInterpolate(((_a = order.status) == null ? void 0 : _a.toUpperCase()) || "APPROVED")}) </option>`);
            });
            _push2(`<!--]--></select>`);
            if (__props.errors.sales_order_id) {
              _push2(`<div class="text-red-500 text-sm mt-1" data-v-991a22e9${_scopeId}>${ssrInterpolate(__props.errors.sales_order_id)}</div>`);
            } else {
              _push2(`<!---->`);
            }
            if (unref(form).sales_order_id && !isAdditionalMode.value) {
              _push2(`<div class="mt-2 p-2 bg-green-50 border border-green-200 rounded-md" data-v-991a22e9${_scopeId}><p class="text-xs text-green-700" data-v-991a22e9${_scopeId}> Data loaded automatically from the Sales Order: ${ssrInterpolate(mainItems.value.length)} main items, ${ssrInterpolate(reimbursementItems.value.length)} reimbursement items, ${ssrInterpolate(operationalCosts.value.length)} operational costs </p></div>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`</div><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>Invoice Type</label><div class="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-100 text-gray-600" data-v-991a22e9${_scopeId}>`);
            if (isAdditionalMode.value) {
              _push2(`<span data-v-991a22e9${_scopeId}>Additional Invoice (Manual Items)</span>`);
            } else {
              _push2(`<span data-v-991a22e9${_scopeId}>Combined Invoice (Main + Reimbursement)</span>`);
            }
            _push2(`</div><input type="hidden"${ssrRenderAttr("value", unref(form).invoice_type)} data-v-991a22e9${_scopeId}></div></div>`);
            if (isAdditionalMode.value) {
              _push2(`<div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6" data-v-991a22e9${_scopeId}><div class="md:col-span-2" data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>Additional Reason</label><textarea rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500" placeholder="Jelaskan alasan additional invoice..." data-v-991a22e9${_scopeId}>${ssrInterpolate(unref(form).additional_reason)}</textarea>`);
              if (__props.errors.additional_reason) {
                _push2(`<div class="text-red-500 text-sm mt-1" data-v-991a22e9${_scopeId}>${ssrInterpolate(__props.errors.additional_reason)}</div>`);
              } else {
                _push2(`<!---->`);
              }
              _push2(`</div></div>`);
            ...
            _push2(`</div><div class="bg-white rounded-lg shadow-sm p-6 border border-sage-200" data-v-991a22e9${_scopeId}><h3 class="text-lg font-semibold text-sage-800 mb-4" data-v-991a22e9${_scopeId}>Invoice Details</h3><div class="grid grid-cols-1 md:grid-cols-2 gap-6" data-v-991a22e9${_scopeId}><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>Invoice Date</label><input type="date"${ssrRenderAttr("value", unref(form).invoice_date)} class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500" required data-v-991a22e9${_scopeId}>`);
            if (__props.errors.invoice_date) {
              _push2(`<div class="text-red-500 text-sm mt-1" data-v-991a22e9${_scopeId}>${ssrInterpolate(__props.errors.invoice_date)}</div>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`</div><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>Term (Days)</label><input type="number"${ssrRenderAttr("value", unref(form).term_days)} min="1" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500" required data-v-991a22e9${_scopeId}>`);
            if (__props.errors.term_days) {
              _push2(`<div class="text-red-500 text-sm mt-1" data-v-991a22e9${_scopeId}>${ssrInterpolate(__props.errors.term_days)}</div>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`</div></div></div><div class="bg-white rounded-lg shadow-sm p-6 border border-sage-200" data-v-991a22e9${_scopeId}><h3 class="text-lg font-semibold text-sage-800 mb-4" data-v-991a22e9${_scopeId}>Shipment Details</h3><div class="grid grid-cols-1 md:grid-cols-2 gap-6" data-v-991a22e9${_scopeId}><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>Shipper</label><input type="text"${ssrRenderAttr("value", unref(form).shipper)} class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500" data-v-991a22e9${_scopeId}></div><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>Consignee</label><input type="text"${ssrRenderAttr("value", unref(form).consignee)} class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500" data-v-991a22e9${_scopeId}></div><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>AWB/BL No.</label><input type="text"${ssrRenderAttr("value", unref(form).awb_bl_no)} class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500" data-v-991a22e9${_scopeId}></div><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>MAWB/OBL No.</label><input type="text"${ssrRenderAttr("value", unref(form).mawb_obl_no)} class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500" data-v-991a22e9${_scopeId}></div><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>Vessel</label><input type="text"${ssrRenderAttr("value", unref(form).vessel)} class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500" data-v-991a22e9${_scopeId}></div><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>Flight/VOY</label><input type="text"${ssrRenderAttr("value", unref(form).flight_voy)} class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500" data-v-991a22e9${_scopeId}></div><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>POL/POD</label><input type="text"${ssrRenderAttr("value", unref(form).pol_pod)} class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500" data-v-991a22e9${_scopeId}></div><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>Origin</label><input type="text"${ssrRenderAttr("value", unref(form).origin)} class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500" data-v-991a22e9${_scopeId}></div><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>Destination</label><input type="text"${ssrRenderAttr("value", unref(form).destination)} class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500" data-v-991a22e9${_scopeId}></div><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>ETD</label><input type="date"${ssrRenderAttr("value", unref(form).etd)} class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500" data-v-991a22e9${_scopeId}></div><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>ETA</label><input type="date"${ssrRenderAttr("value", unref(form).eta)} class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500" data-v-991a22e9${_scopeId}></div><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>Gross Weight (KG)</label><input type="number"${ssrRenderAttr("value", unref(form).gross_weight)} step="0.0001" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500" data-v-991a22e9${_scopeId}></div><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>Net Weight (KG)</label><input type="number"${ssrRenderAttr("value", unref(form).net_weight)} step="0.0001" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500" data-v-991a22e9${_scopeId}></div><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>Volume</label><input type="text"${ssrRenderAttr("value", unref(form).volume)} class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500" placeholder="e.g., 10.5 M³" data-v-991a22e9${_scopeId}></div><div class="grid grid-cols-2 gap-3" data-v-991a22e9${_scopeId}><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>No of Packages</label><input type="number"${ssrRenderAttr("value", unref(form).no_of_packages)} min="0" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500" data-v-991a22e9${_scopeId}></div><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>Package Unit</label><select class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500" data-v-991a22e9${_scopeId}><option value="" data-v-991a22e9${ssrIncludeBooleanAttr(Array.isArray(unref(form).package_unit) ? ssrLooseContain(unref(form).package_unit, "") : ssrLooseEqual(unref(form).package_unit, "")) ? " selected" : ""}${_scopeId}>Select Unit</option><!--[-->`);
            ssrRenderList(__props.packageUnits, (unit) => {
              _push2(`<option${ssrRenderAttr("value", unit.code)} data-v-991a22e9${ssrIncludeBooleanAttr(Array.isArray(unref(form).package_unit) ? ssrLooseContain(unref(form).package_unit, unit.code) : ssrLooseEqual(unref(form).package_unit, unit.code)) ? " selected" : ""}${_scopeId}>${ssrInterpolate(unit.name)}</option>`);
            });
            _push2(`<!--]--></select></div></div><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>20&#39;/40&#39;/45&#39;</label><input type="text"${ssrRenderAttr("value", unref(form).container_size)} class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500" placeholder="e.g., 20GP, 40GP, 45GP" data-v-991a22e9${_scopeId}></div><div class="md:col-span-2" data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>Container No.</label><input type="text"${ssrRenderAttr("value", unref(form).container_no)} class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500" placeholder="e.g., MSKU2934199" data-v-991a22e9${_scopeId}></div><div class="md:col-span-2" data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>Remarks</label><textarea rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500" data-v-991a22e9${_scopeId}>${ssrInterpolate(unref(form).remarks)}</textarea></div></div></div><div class="bg-white rounded-lg shadow-sm p-6 border border-sage-200" data-v-991a22e9${_scopeId}><h3 class="text-lg font-semibold text-sage-800 mb-4" data-v-991a22e9${_scopeId}>Down Payment (DP)</h3><div class="grid grid-cols-1 gap-6" data-v-991a22e9${_scopeId}><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>Down Payment Amount</label><input type="number"${ssrRenderAttr("value", unref(form).down_payment_amount)} step="0.01" min="0" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500" placeholder="0.00" data-v-991a22e9${_scopeId}>`);
            if (__props.errors.down_payment_amount) {
              _push2(`<div class="text-red-500 text-sm mt-1" data-v-991a22e9${_scopeId}>${ssrInterpolate(__props.errors.down_payment_amount)}</div>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`</div><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>Down Payment Date</label><input type="date"${ssrRenderAttr("value", unref(form).down_payment_date)} class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500" data-v-991a22e9${_scopeId}>`);
            if (__props.errors.down_payment_date) {
              _push2(`<div class="text-red-500 text-sm mt-1" data-v-991a22e9${_scopeId}>${ssrInterpolate(__props.errors.down_payment_date)}</div>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`</div><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>Down Payment Notes</label><textarea rows="2" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500" placeholder="Notes for the down payment..." data-v-991a22e9${_scopeId}>${ssrInterpolate(unref(form).down_payment_notes)}</textarea>`);
            if (__props.errors.down_payment_notes) {
              _push2(`<div class="text-red-500 text-sm mt-1" data-v-991a22e9${_scopeId}>${ssrInterpolate(__props.errors.down_payment_notes)}</div>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`</div></div></div><div class="bg-white rounded-lg shadow-sm p-6 border border-sage-200" data-v-991a22e9${_scopeId}><h3 class="text-lg font-semibold text-sage-800 mb-4" data-v-991a22e9${_scopeId}>VAT (PPN)</h3><div class="grid grid-cols-1 md:grid-cols-2 gap-6" data-v-991a22e9${_scopeId}><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>VAT Rate</label><select class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500" data-v-991a22e9${_scopeId}><option value="" data-v-991a22e9${ssrIncludeBooleanAttr(Array.isArray(unref(form).vat_rate) ? ssrLooseContain(unref(form).vat_rate, "") : ssrLooseEqual(unref(form).vat_rate, "")) ? " selected" : ""}${_scopeId}>No VAT</option><option value="11" data-v-991a22e9${ssrIncludeBooleanAttr(Array.isArray(unref(form).vat_rate) ? ssrLooseContain(unref(form).vat_rate, "11") : ssrLooseEqual(unref(form).vat_rate, "11")) ? " selected" : ""}${_scopeId}>11%</option><option value="1.1" data-v-991a22e9${ssrIncludeBooleanAttr(Array.isArray(unref(form).vat_rate) ? ssrLooseContain(unref(form).vat_rate, "1.1") : ssrLooseEqual(unref(form).vat_rate, "1.1")) ? " selected" : ""}${_scopeId}>1.1%</option></select>`);
            if (__props.errors.vat_rate) {
              _push2(`<div class="text-red-500 text-sm mt-1" data-v-991a22e9${_scopeId}>${ssrInterpolate(__props.errors.vat_rate)}</div>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`</div><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>VAT Amount</label><input type="text"${ssrRenderAttr("value", formatCurrency(calculateVatAmount()))} class="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-100" readonly data-v-991a22e9${_scopeId}></div></div></div><div class="space-y-6" data-v-991a22e9${_scopeId}><div class="bg-white rounded-lg shadow-sm p-6 border border-sage-200" data-v-991a22e9${_scopeId}><div class="flex items-center justify-between mb-4" data-v-991a22e9${_scopeId}><h3 class="text-lg font-semibold text-sage-800" data-v-991a22e9${_scopeId}>Main Invoice Items</h3><div class="flex space-x-2" data-v-991a22e9${_scopeId}>`);
            if (unref(form).sales_order_id && !isAdditionalMode.value) {
              _push2(`<button type="button" class="inline-flex items-center px-3 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors" data-v-991a22e9${_scopeId}><svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" data-v-991a22e9${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" data-v-991a22e9${_scopeId}></path></svg> Load from SO </button>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`<button type="button" class="inline-flex items-center px-3 py-2 bg-sage-600 text-white rounded-lg hover:bg-sage-700 transition-colors" data-v-991a22e9${_scopeId}><svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" data-v-991a22e9${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" data-v-991a22e9${_scopeId}></path></svg> Add Item </button></div></div>`);
            if (mainItems.value.length === 0) {
              _push2(`<div class="text-gray-500 text-center py-8 border-2 border-dashed border-gray-300 rounded-lg" data-v-991a22e9${_scopeId}><div class="flex flex-col items-center" data-v-991a22e9${_scopeId}><svg class="w-12 h-12 text-gray-300 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" data-v-991a22e9${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" data-v-991a22e9${_scopeId}></path></svg><p data-v-991a22e9${_scopeId}>No main invoice items yet</p><p class="text-sm" data-v-991a22e9${_scopeId}>Click &quot;Add Item&quot; to add an item</p></div></div>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`<div class="space-y-4" data-v-991a22e9${_scopeId}><!--[-->`);
            ssrRenderList(mainItems.value, (item, index) => {
              _push2(`<div class="border border-gray-200 rounded-lg p-4" data-v-991a22e9${_scopeId}><div class="flex items-center justify-between mb-4" data-v-991a22e9${_scopeId}><h4 class="font-medium text-gray-900" data-v-991a22e9${_scopeId}>Item ${ssrInterpolate(index + 1)}</h4><button type="button" class="text-red-600 hover:text-red-800" data-v-991a22e9${_scopeId}><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" data-v-991a22e9${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" data-v-991a22e9${_scopeId}></path></svg></button></div><div class="grid grid-cols-1 md:grid-cols-6 gap-4" data-v-991a22e9${_scopeId}><div class="md:col-span-2" data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>Description</label><input type="text"${ssrRenderAttr("value", item.description)} class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500" required data-v-991a22e9${_scopeId}></div><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>Qty</label><input type="number"${ssrRenderAttr("value", item.quantity)} step="0.01" min="0.01" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500" required data-v-991a22e9${_scopeId}></div><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>Unit</label><input type="text"${ssrRenderAttr("value", item.unit)} class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500" required data-v-991a22e9${_scopeId}></div><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>Rate</label><input type="text"${ssrRenderAttr("value", item.rate)} placeholder="0 (e.g., 2,500 or 2500)" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500" required data-v-991a22e9${_scopeId}></div><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-2" data-v-991a22e9${_scopeId}>Amount</label><input type="text"${ssrRenderAttr("value", formatCurrency(item.amount || 0))} class="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-100" readonly data-v-991a22e9${_scopeId}></div></div></div>`);
            });
            _push2(`<!--]--></div>`);
            if (mainItems.value.length > 0) {
              _push2(`<div class="flex justify-center mt-6 pt-4 border-t border-gray-200" data-v-991a22e9${_scopeId}><div class="flex space-x-2" data-v-991a22e9${_scopeId}>`);
              if (unref(form).sales_order_id && !isAdditionalMode.value) {
                _push2(`<button type="button" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors" data-v-991a22e9${_scopeId}><svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" data-v-991a22e9${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" data-v-991a22e9${_scopeId}></path></svg> Load from SO </button>`);
              } else {
                _push2(`<!---->`);
              }
              _push2(`<button type="button" class="inline-flex items-center px-4 py-2 bg-sage-600 text-white rounded-lg hover:bg-sage-700 transition-colors" data-v-991a22e9${_scopeId}><svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" data-v-991a22e9${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" data-v-991a22e9${_scopeId}></path></svg> Add Another Item </button></div></div>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`</div><div class="bg-white rounded-lg shadow-sm p-6 border border-sage-200" data-v-991a22e9${_scopeId}><div class="flex items-center justify-between mb-4" data-v-991a22e9${_scopeId}><h3 class="text-lg font-semibold text-sage-800" data-v-991a22e9${_scopeId}>Reimbursement Items</h3><button type="button" class="inline-flex items-center px-3 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition-colors" data-v-991a22e9${_scopeId}><svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" data-v-991a22e9${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" data-v-991a22e9${_scopeId}></path></svg> Add Reimbursement Item </button></div>`);
            if (reimbursementItems.value.length === 0) {
              _push2(`<div class="text-gray-500 text-center py-8 border-2 border-dashed border-gray-300 rounded-lg" data-v-991a22e9${_scopeId}><div class="flex flex-col items-center" data-v-991a22e9${_scopeId}><svg class="w-12 h-12 text-gray-300 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" data-v-991a22e9${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" data-v-991a22e9${_scopeId}></path></svg><p data-v-991a22e9${_scopeId}>No reimbursement items yet</p><p class="text-sm" data-v-991a22e9${_scopeId}>Click &quot;Add Reimbursement Item&quot; to add an item</p></div></div>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`<div class="space-y-4" data-v-991a22e9${_scopeId}><!--[-->`);
            ssrRenderList(reimbursementItems.value, (item, index) => {
              _push2(`<div class="border border-gray-200 rounded-lg p-4" data-v-991a22e9${_scopeId}><div class="flex justify-between items-center mb-3" data-v-991a22e9${_scopeId}><h4 class="font-medium text-gray-700" data-v-991a22e9${_scopeId}>Reimbursement Item #${ssrInterpolate(index + 1)}</h4><button type="button" class="text-red-600 hover:text-red-800" data-v-991a22e9${_scopeId}><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" data-v-991a22e9${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" data-v-991a22e9${_scopeId}></path></svg></button></div><div class="grid grid-cols-1 gap-4" data-v-991a22e9${_scopeId}><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-1" data-v-991a22e9${_scopeId}>Item Number/Ref</label><input${ssrRenderAttr("value", item.item_ref)} type="text" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-sage-500 focus:border-sage-500" placeholder="e.g., REIMB-001" data-v-991a22e9${_scopeId}></div><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-1" data-v-991a22e9${_scopeId}>Currency</label><select class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-sage-500 focus:border-sage-500" data-v-991a22e9${_scopeId}><option value="IDR" data-v-991a22e9${ssrIncludeBooleanAttr(Array.isArray(item.currency) ? ssrLooseContain(item.currency, "IDR") : ssrLooseEqual(item.currency, "IDR")) ? " selected" : ""}${_scopeId}>IDR</option><option value="USD" data-v-991a22e9${ssrIncludeBooleanAttr(Array.isArray(item.currency) ? ssrLooseContain(item.currency, "USD") : ssrLooseEqual(item.currency, "USD")) ? " selected" : ""}${_scopeId}>USD</option><option value="EUR" data-v-991a22e9${ssrIncludeBooleanAttr(Array.isArray(item.currency) ? ssrLooseContain(item.currency, "EUR") : ssrLooseEqual(item.currency, "EUR")) ? " selected" : ""}${_scopeId}>EUR</option><option value="SGD" data-v-991a22e9${ssrIncludeBooleanAttr(Array.isArray(item.currency) ? ssrLooseContain(item.currency, "SGD") : ssrLooseEqual(item.currency, "SGD")) ? " selected" : ""}${_scopeId}>SGD</option></select></div></div><div class="mt-3" data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-1" data-v-991a22e9${_scopeId}>Description</label><textarea rows="2" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-sage-500 focus:border-sage-500 resize-none" placeholder="e.g., Trucking cost from warehouse to port" required data-v-991a22e9${_scopeId}>${ssrInterpolate(item.description)}</textarea></div><div class="mt-3" data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-1" data-v-991a22e9${_scopeId}> Vendor / Recipient </label>`);
              ...
              _push2(`<p class="text-xs text-gray-600 mt-1" data-v-991a22e9${_scopeId}> Select a vendor if this cost will be paid to an external vendor; leave blank for internal </p></div><div class="grid grid-cols-1 gap-4 mt-3" data-v-991a22e9${_scopeId}><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-1" data-v-991a22e9${_scopeId}>Quantity</label><input${ssrRenderAttr("value", item.quantity)} type="number" step="0.01" min="0.01" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-sage-500 focus:border-sage-500" placeholder="1" required data-v-991a22e9${_scopeId}></div><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-1" data-v-991a22e9${_scopeId}>Unit Rate</label><input${ssrRenderAttr("value", item.rate)} type="text" placeholder="0 (e.g., 500,000 or 500000)" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-sage-500 focus:border-sage-500" required data-v-991a22e9${_scopeId}></div><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-gray-700 mb-1" data-v-991a22e9${_scopeId}>Total Amount</label><input type="text"${ssrRenderAttr("value", formatCurrency(item.amount || 0, item.currency || "IDR"))} class="w-full px-3 py-2 border border-gray-300 rounded-md bg-gray-100 text-gray-600" readonly data-v-991a22e9${_scopeId}></div></div></div>`);
            });
            _push2(`<!--]--></div>`);
            if (reimbursementItems.value.length > 0) {
              _push2(`<div class="flex justify-center mt-6 pt-4 border-t border-orange-200" data-v-991a22e9${_scopeId}><button type="button" class="inline-flex items-center px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition-colors" data-v-991a22e9${_scopeId}><svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" data-v-991a22e9${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" data-v-991a22e9${_scopeId}></path></svg> Add Another Reimbursement Item </button></div>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`</div><div class="bg-white rounded-lg shadow-sm p-6 border border-sage-200 border-l-4 border-l-red-500" data-v-991a22e9${_scopeId}><div class="flex items-center justify-between mb-4" data-v-991a22e9${_scopeId}><div data-v-991a22e9${_scopeId}><h3 class="text-lg font-semibold text-red-800" data-v-991a22e9${_scopeId}>Operational Costs (Internal)</h3><p class="text-sm text-red-600" data-v-991a22e9${_scopeId}>These costs will not appear on the customer invoice and will reduce profit</p>`);
            if (operationalCosts.value.some((cost) => cost.auto_generated)) {
              _push2(`<div class="mt-2 p-3 bg-blue-50 border border-blue-200 rounded-lg" data-v-991a22e9${_scopeId}><div class="flex items-start space-x-2" data-v-991a22e9${_scopeId}><svg class="w-4 h-4 text-blue-600 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" data-v-991a22e9${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" data-v-991a22e9${_scopeId}></path></svg><div class="text-xs text-blue-800" data-v-991a22e9${_scopeId}><strong data-v-991a22e9${_scopeId}>Info:</strong> Costs with the label <span class="inline-flex items-center px-1 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-800" data-v-991a22e9${_scopeId}>Auto from SO</span> were loaded automatically from the Sales Order. You can edit the amount or description as needed. </div></div></div>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`</div><button type="button" class="inline-flex items-center px-3 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors" data-v-991a22e9${_scopeId}><svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" data-v-991a22e9${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" data-v-991a22e9${_scopeId}></path></svg> Add Operational Cost </button></div>`);
            if (operationalCosts.value.length === 0) {
              _push2(`<div class="text-gray-500 text-center py-8 border-2 border-dashed border-red-300 rounded-lg bg-red-50" data-v-991a22e9${_scopeId}><div class="flex flex-col items-center" data-v-991a22e9${_scopeId}><svg class="w-12 h-12 text-red-300 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" data-v-991a22e9${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" data-v-991a22e9${_scopeId}></path></svg><p class="text-red-600" data-v-991a22e9${_scopeId}>No operational costs yet</p><p class="text-sm text-red-500" data-v-991a22e9${_scopeId}>Example: document delivery, escort fees, parking, etc.</p></div></div>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`<div class="space-y-4" data-v-991a22e9${_scopeId}><!--[-->`);
            ssrRenderList(operationalCosts.value, (cost, index) => {
              ...
              ])}" data-v-991a22e9${_scopeId}><div class="flex items-center justify-between mb-4" data-v-991a22e9${_scopeId}><div class="flex items-center space-x-2" data-v-991a22e9${_scopeId}><h4 class="${ssrRenderClass([
                "font-medium",
                cost.auto_generated ? "text-blue-800" : "text-red-800"
              ])}" data-v-991a22e9${_scopeId}> Operational Cost ${ssrInterpolate(index + 1)}</h4>`);
              if (cost.auto_generated) {
                _push2(`<span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800" data-v-991a22e9${_scopeId}><svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" data-v-991a22e9${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z" data-v-991a22e9${_scopeId}></path></svg> Auto from SO </span>`);
              } else {
                _push2(`<span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800" data-v-991a22e9${_scopeId}><svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" data-v-991a22e9${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" data-v-991a22e9${_scopeId}></path></svg> Manual </span>`);
              }
              _push2(`</div><button type="button" class="${ssrRenderClass([
                "hover:text-red-800",
                cost.auto_generated ? "text-blue-600" : "text-red-600"
              ])}" data-v-991a22e9${_scopeId}><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" data-v-991a22e9${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" data-v-991a22e9${_scopeId}></path></svg></button></div><div class="mb-4" data-v-991a22e9${_scopeId}><label class="${ssrRenderClass([
                "block text-sm font-medium mb-2",
                cost.auto_generated ? "text-blue-700" : "text-red-700"
              ])}" data-v-991a22e9${_scopeId}> Cost Category `);
              if (cost.source !== "vendor_breakdown_buying") {
                _push2(`<span data-v-991a22e9${_scopeId}>*</span>`);
              } else {
                _push2(`<!---->`);
              }
              _push2(`</label>`);
              if (cost.source === "vendor_breakdown_buying") {
                _push2(`<div class="px-3 py-2 border border-blue-200 rounded-lg bg-blue-50 text-sm text-blue-800" data-v-991a22e9${_scopeId}> Buying cost (COGS) is loaded from the Sales Order. Category is not required. </div>`);
              } else {
                _push2(ssrRenderComponent(_sfc_main$2, {
              ...
              _push2(`</div><div class="mb-4" data-v-991a22e9${_scopeId}><label class="${ssrRenderClass([
                "block text-sm font-medium mb-2",
                cost.auto_generated ? "text-blue-700" : "text-red-700"
              ])}" data-v-991a22e9${_scopeId}> Vendor / Recipient </label>`);
              _push2(ssrRenderComponent(_sfc_main$2, {
              ...
              _push2(`<p class="${ssrRenderClass([
                "text-xs mt-1",
                cost.auto_generated ? "text-blue-600" : "text-red-600"
              ])}" data-v-991a22e9${_scopeId}> Select a vendor if this cost will be paid to an external vendor; leave blank for internal </p></div><div class="grid grid-cols-1 md:grid-cols-4 gap-4" data-v-991a22e9${_scopeId}><div class="md:col-span-2" data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-red-700 mb-2" data-v-991a22e9${_scopeId}>Description</label><input type="text"${ssrRenderAttr("value", cost.description)} class="w-full px-3 py-2 border border-red-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500" placeholder="e.g., document delivery, escort fees, refreshments" required data-v-991a22e9${_scopeId}></div><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-red-700 mb-2" data-v-991a22e9${_scopeId}>Cost</label><input type="text"${ssrRenderAttr("value", cost.rate)} placeholder="0 (e.g., 50,000 or 50000)" class="w-full px-3 py-2 border border-red-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500" required data-v-991a22e9${_scopeId}></div><div data-v-991a22e9${_scopeId}><label class="block text-sm font-medium text-red-700 mb-2" data-v-991a22e9${_scopeId}>Total</label><input type="text"${ssrRenderAttr("value", formatCurrency(cost.amount || 0))} class="w-full px-3 py-2 border border-red-300 rounded-lg bg-red-100" readonly data-v-991a22e9${_scopeId}></div></div></div>`);
            });
            _push2(`<!--]--></div>`);
            if (operationalCosts.value.length > 0) {
              _push2(`<div class="mt-4 pt-4 border-t border-red-200" data-v-991a22e9${_scopeId}><div class="flex justify-between items-center text-sm" data-v-991a22e9${_scopeId}><span class="font-medium text-red-700" data-v-991a22e9${_scopeId}>Total Operational Costs:</span><span class="font-bold text-red-800" data-v-991a22e9${_scopeId}>${ssrInterpolate(formatCurrency(calculateOperationalTotal()))}</span></div></div>`);
            } else {
              _push2(`<!---->`);
            }
            if (operationalCosts.value.length > 0) {
              _push2(`<div class="flex justify-center mt-6 pt-4 border-t border-red-200" data-v-991a22e9${_scopeId}><button type="button" class="inline-flex items-center px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors" data-v-991a22e9${_scopeId}><svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" data-v-991a22e9${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" data-v-991a22e9${_scopeId}></path></svg> Add Another Operational Cost </button></div>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`</div></div>`);
            if (operationalCosts.value.length > 0) {
              _push2(`<div class="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg shadow-sm p-6 border border-blue-200" data-v-991a22e9${_scopeId}><h3 class="text-lg font-semibold text-blue-800 mb-4" data-v-991a22e9${_scopeId}>Profit Summary</h3><div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-sm" data-v-991a22e9${_scopeId}><div class="bg-white rounded-lg p-4 border border-blue-200" data-v-991a22e9${_scopeId}><div class="text-blue-600 font-medium" data-v-991a22e9${_scopeId}>Gross Revenue</div><div class="text-xl font-bold text-blue-800" data-v-991a22e9${_scopeId}>${ssrInterpolate(formatCurrency(calculateGrossRevenue()))}</div><div class="text-xs text-blue-500" data-v-991a22e9${_scopeId}>Billable items only</div></div><div class="bg-white rounded-lg p-4 border border-red-200" data-v-991a22e9${_scopeId}><div class="text-red-600 font-medium" data-v-991a22e9${_scopeId}>Operational Costs</div><div class="text-xl font-bold text-red-800" data-v-991a22e9${_scopeId}>${ssrInterpolate(formatCurrency(calculateOperationalTotal()))}</div><div class="text-xs text-red-500" data-v-991a22e9${_scopeId}>Internal costs only</div></div><div class="bg-white rounded-lg p-4 border border-green-200" data-v-991a22e9${_scopeId}><div class="text-green-600 font-medium" data-v-991a22e9${_scopeId}>Net Profit</div><div class="text-xl font-bold text-green-800" data-v-991a22e9${_scopeId}>${ssrInterpolate(formatCurrency(calculateNetProfit()))}</div><div class="text-xs text-green-500" data-v-991a22e9${_scopeId}>Gross - Operational</div></div><div class="bg-white rounded-lg p-4 border border-purple-200" data-v-991a22e9${_scopeId}><div class="text-purple-600 font-medium" data-v-991a22e9${_scopeId}>Profit Margin</div><div class="text-xl font-bold text-purple-800" data-v-991a22e9${_scopeId}>${ssrInterpolate(calculateProfitMargin())}%</div><div class="text-xs text-purple-500" data-v-991a22e9${_scopeId}>Net profit percentage</div></div></div><div class="mt-6 bg-white rounded-lg p-4 border border-blue-200" data-v-991a22e9${_scopeId}><h4 class="text-sm font-semibold text-blue-800 mb-3" data-v-991a22e9${_scopeId}>Calculation Details</h4><div class="space-y-2 text-sm text-gray-700" data-v-991a22e9${_scopeId}><div class="flex items-center justify-between" data-v-991a22e9${_scopeId}><span data-v-991a22e9${_scopeId}>Main Invoice</span><span class="font-medium text-gray-900" data-v-991a22e9${_scopeId}>${ssrInterpolate(formatCurrency(calculateGrossRevenue()))}</span></div><div class="flex items-center justify-between" data-v-991a22e9${_scopeId}><span data-v-991a22e9${_scopeId}>VAT</span><span class="font-medium text-gray-900" data-v-991a22e9${_scopeId}>${ssrInterpolate(formatCurrency(calculateVatAmount()))}</span></div><div class="flex items-center justify-between py-1 border-t border-b border-blue-100" data-v-991a22e9${_scopeId}><span data-v-991a22e9${_scopeId}>Total Main Invoice + VAT</span><span class="font-semibold text-blue-900" data-v-991a22e9${_scopeId}>${ssrInterpolate(formatCurrency(calculateGrossRevenue() + calculateVatAmount()))}</span></div><div class="flex items-center justify-between" data-v-991a22e9${_scopeId}><span data-v-991a22e9${_scopeId}>Reimbursement</span><span class="font-medium text-gray-900" data-v-991a22e9${_scopeId}>${ssrInterpolate(formatCurrency(calculateReimbursementTotal()))}</span></div><div class="flex items-center justify-between py-1 border-t border-b border-blue-100" data-v-991a22e9${_scopeId}><span data-v-991a22e9${_scopeId}>Total Invoice + Reimbursement</span><span class="font-semibold text-blue-900" data-v-991a22e9${_scopeId}>${ssrInterpolate(formatCurrency(calculateGrossRevenue() + calculateVatAmount() + calculateReimbursementTotal()))}</span></div><div class="flex items-center justify-between pt-2 border-t border-blue-100" data-v-991a22e9${_scopeId}><span data-v-991a22e9${_scopeId}>Operational Costs</span><span class="font-semibold text-red-700" data-v-991a22e9${_scopeId}>${ssrInterpolate(formatCurrency(calculateOperationalTotal()))}</span></div><div class="flex items-center justify-between" data-v-991a22e9${_scopeId}><span data-v-991a22e9${_scopeId}>Net Profit</span><span class="font-semibold text-green-700" data-v-991a22e9${_scopeId}>${ssrInterpolate(formatCurrency(calculateNetProfit()))}</span></div><div class="flex items-center justify-between" data-v-991a22e9${_scopeId}><span data-v-991a22e9${_scopeId}>Profit Percentage</span><span class="font-semibold text-purple-700" data-v-991a22e9${_scopeId}>${ssrInterpolate(calculateProfitMargin())}%</span></div></div></div></div>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`<div class="bg-white rounded-lg shadow-sm p-6 border border-sage-200" data-v-991a22e9${_scopeId}><div class="flex items-center justify-between" data-v-991a22e9${_scopeId}><div class="text-lg font-semibold text-sage-800" data-v-991a22e9${_scopeId}> Total: ${ssrInterpolate(formatCurrency(calculateTotal()))}</div><div class="flex space-x-4" data-v-991a22e9${_scopeId}>`);
            _push2(ssrRenderComponent(unref(Link), {
              href: unref(route)("admin-keuangan.invoices.index"),
            ...
            _push2(`<button type="submit"${ssrIncludeBooleanAttr(unref(form).processing) ? " disabled" : ""} class="px-6 py-2 bg-sage-600 text-white rounded-lg hover:bg-sage-700 transition-colors disabled:opacity-50" data-v-991a22e9${_scopeId}>${ssrInterpolate(unref(form).processing ? "Saving..." : "Save Invoice")}</button></div></div></div></form></div>`);
                  ...
                  createVNode("div", { class: "bg-white rounded-lg shadow-sm p-6 border border-sage-200" }, [
                    ...
                    createVNode("div", { class: "grid grid-cols-1 md:grid-cols-2 gap-6" }, [
                      createVNode("div", null, [
                        ...
                        unref(form).sales_order_id && !isAdditionalMode.value ? (openBlock(), createBlock("div", {
                          key: 1,
                      ...
                      createVNode("div", null, [
                        ...
                        createVNode("div", { class: "w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-100 text-gray-600" }, [
                          isAdditionalMode.value ? (openBlock(), createBlock("span", { key: 0 }, "Additional Invoice (Manual Items)")) : (openBlock(), createBlock("span", { key: 1 }, "Combined Invoice (Main + Reimbursement)"))
                        ]),
                    ...
                    isAdditionalMode.value ? (openBlock(), createBlock("div", {
                      key: 0,
                      class: "mt-6 grid grid-cols-1 md:grid-cols-2 gap-6"
                    }, [
                      createVNode("div", { class: "md:col-span-2" }, [
                        createVNode("label", { class: "block text-sm font-medium text-gray-700 mb-2" }, "Additional Reason"),
                        withDirectives(createVNode("textarea", {
                          "onUpdate:modelValue": ($event) => unref(form).additional_reason = $event,
                          rows: "3",
                          class: "w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500",
                          placeholder: "Jelaskan alasan additional invoice..."
                        }, null, 8, ["onUpdate:modelValue"]), [
                          [vModelText, unref(form).additional_reason]
                        ]),
                        __props.errors.additional_reason ? (openBlock(), createBlock("div", {
                          key: 0,
                          class: "text-red-500 text-sm mt-1"
                        }, toDisplayString(__props.errors.additional_reason), 1)) : createCommentVNode("", true)
                      ])
                    ])) : createCommentVNode("", true)
                  ]),
                  createVNode("div", { class: "bg-white rounded-lg shadow-sm p-6 border border-sage-200" }, [
                    ...
                    createVNode("div", { class: "bg-white rounded-lg shadow-sm p-6 border border-sage-200" }, [
                      createVNode("div", { class: "flex items-center justify-between mb-4" }, [
                        ...
                        createVNode("div", { class: "flex space-x-2" }, [
                          unref(form).sales_order_id && !isAdditionalMode.value ? (openBlock(), createBlock("button", {
                            key: 0,
                            type: "button",
                      ...
                      createVNode("div", { class: "space-y-4" }, [
                        (openBlock(true), createBlock(Fragment, null, renderList(reimbursementItems.value, (item, index) => {
                          return openBlock(), createBlock("div", {
                            ...
                            createVNode("div", { class: "grid grid-cols-1 gap-4 mt-3" }, [
                              createVNode("div", null, [
...
const Create = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-991a22e9"]]);
export {
  Create as default
};
