...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const indexUrl = computed(
      () => `${route("admin-keuangan.chart-of-accounts.index")}${buildQueryString(props.filters || {})}`
    );
    const updateUrl = computed(
      () => `${route("admin-keuangan.chart-of-accounts.update", props.account.id)}${buildQueryString(props.filters || {})}`
    );
    const form = useForm({
