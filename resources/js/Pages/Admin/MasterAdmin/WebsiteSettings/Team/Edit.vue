          ...
          >
            <div>
              ...
              <p class="text-sage-600">
                Perbarui informasi anggota tim yang akan ditampilkan di website
              </p>
            </div>
        ...
        >
          <div class="px-6 py-4 border-b border-sage-200">
            ...
            <p class="text-sm text-sage-600 mt-1">
              Lengkapi detail anggota tim baru
            </p>
          </div>
          ...
          <form @submit.prevent="submitForm" class="p-6">
            <div class="space-y-6">
              ...
              <div>
                ...
                <div v-if="props.member?.photo_path && !photoPreview" class="mb-4">
                  <p class="text-sm text-gray-600 mb-2">Foto saat ini:</p>
                  <img
                    :src="`/storage/${props.member.photo_path}`"
                ...
                <div v-if="photoPreview" class="mt-4 text-center">
                  <img
                    :src="photoPreview"
                    alt="Preview"
                    class="w-32 h-32 object-cover rounded-full border mx-auto"
                  />
                </div>
...
<span v-else>Update Anggota Tim</span>
              </button>
            </div>
          </form>
        </div>
...
// Form data - populate with existing member data
const form = reactive({
  name: props.member?.name || "",
  position: props.member?.position || "",
  phone_number: props.member?.phone_number || "",
  photo_path: null, // For new photo upload
  order_index: props.member?.order_index || 0,
...
const submitForm = async () => {
  ...
  try {
    ...
    // Submit form using router for update
    router.post(`/master-admin/website-settings/team/${props.member.id}`, formData, {
      onSuccess: () => {
