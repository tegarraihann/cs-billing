      ...
      <div class="px-4 sm:px-6 lg:ml-64 lg:px-8">
        <div class="flex justify-between items-center h-16">
          ...
          <div class="flex-1 lg:flex-none">
            <h1 class="text-lg sm:text-xl font-semibold text-sage-800">Edit Service</h1>
          </div>
      ...
      <div class="p-4 sm:p-6 lg:p-8">
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
              ...
              <li>
                <div class="flex items-center">
                  ...
                  <a :href="serviceRoute" class="ml-1 text-sage-600 hover:text-sage-800 md:ml-2">Kelola Service</a>
                </div>
              </li>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6 mb-6 border border-sage-200">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 class="text-2xl font-bold text-sage-800 mb-2">Edit Service</h2>
              <p class="text-sage-600">Perbarui informasi service "{{ service?.title }}"</p>
            </div>
            <div class="mt-4 sm:mt-0">
              <a :href="serviceRoute" class="inline-flex items-center px-4 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600 transition-colors">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-sage-200 overflow-hidden">
          <div class="px-6 py-4 border-b border-sage-200">
            <h3 class="text-lg font-semibold text-sage-800">Informasi Service</h3>
          ...
          <form @submit.prevent="submitForm" class="p-6">
            <div class="space-y-6">
              ...
              <div v-if="service?.image_path">
                <label class="block text-sm font-medium text-sage-700 mb-2">Gambar Saat Ini</label>
                <div class="mb-4">
                  <img :src="`/storage/${service.image_path}`" :alt="service.title" class="w-32 h-32 object-cover rounded border"/>
                </div>
              </div>

              <!-- Service Image Upload -->
              <div>
                <label for="image_path" class="block text-sm font-medium text-sage-700 mb-2">
                  {{ service?.image_path ? 'Ganti Gambar Service' : 'Gambar Service' }}
                </label>
                <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
                  <div class="space-y-1 text-center">
                    ...
                    <div class="flex text-sm text-gray-600">
                      <label for="image_path" class="relative cursor-pointer bg-white rounded-md font-medium text-sage-600 hover:text-sage-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-sage-500">
              ...
              <div v-if="service?.icon_path">
                <label class="block text-sm font-medium text-sage-700 mb-2">Icon Saat Ini</label>
                <div class="mb-4">
                  <img :src="`/storage/${service.icon_path}`" :alt="service.title" class="w-16 h-16 object-cover rounded border"/>
                </div>
              </div>

              <!-- Icon Upload -->
              <div>
                <label for="icon_path" class="block text-sm font-medium text-sage-700 mb-2">
                  {{ service?.icon_path ? 'Ganti Icon Service' : 'Icon Service' }}
                </label>
                <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
                  <div class="space-y-1 text-center">
                    <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21a4 4 0 01-4-4V5a4 4 0 014-4h2M7 21h10a4 4 0 004-4V5a4 4 0 00-4-4H9M7 21V8a3 3 0 013-3h4a3 3 0 013 3v13"/>
                    </svg>
                    <div class="flex text-sm text-gray-600">
                      <label for="icon_path" class="relative cursor-pointer bg-white rounded-md font-medium text-sage-600 hover:text-sage-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-sage-500">
                    ...
                    <p class="text-xs text-gray-500">PNG, JPG, SVG hingga 2MB</p>
                  </div>
                </div>
                <div v-if="iconPreview" class="mt-4">
                  <p class="text-sm text-gray-600 mb-2">Preview icon baru:</p>
                  <img :src="iconPreview" alt="Icon Preview" class="w-16 h-16 object-cover rounded border"/>
                </div>
...
// Form data - populate with existing service data
const form = reactive({
...
const submitForm = async () => {
  ...
  try {
    ...
    router.post(route("masteradmin.website-settings.service.update", props.service.id), formData, {
      onSuccess: () => {
