                ...
                <div class="bg-white rounded-lg shadow-sm p-6 border border-sage-200">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select v-model="form.sales_order_id"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <input type="number" v-model="form.no_of_packages"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500" />
                            </div>
                            <div>
                ...
                <div class="bg-white rounded-lg shadow-sm p-6 border border-sage-200">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            ...
                            <div v-if="errors.vat_rate" class="text-red-500 text-sm mt-1">
                                {{ errors.vat_rate }}
                            </div>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">VAT Amount</label>
                            <input type="text" :value="formatCurrency(calculateVatAmount())"
                            ...
                            <div v-for="(item, index) in mainItems" :key="'main-' + index"
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-6 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <input type="number" v-model="item.rate" @input="calculateMainAmount(index)"
                                            step="0.01" min="0"
                                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-sage-500"
                                            required />
                                    </div>
                            ...
                            <div v-for="(item, index) in reimbursementItems" :key="'reimb-' + index"
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3">
                                    <div>
                                        ...
                                        <select v-model="item.vendor_id"
                                            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-sage-500 focus:border-sage-500">
                                            <option :value="null">-- Divisi Operational / Internal --</option>
                                            <option v-for="vendor in vendorOptions" :key="vendor.id" :value="vendor.id">
                                                {{ vendor.nama_vendor }}
                                            </option>
                                        </select>
                                        <p class="text-xs text-gray-500 mt-1">Kosongkan jika biaya ditanggung internal
                                            / Divisi Operational.</p>
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-3">
                                    ...
                                    <div>
                                        ...
                                        <input v-model="item.rate" @input="calculateReimbursementAmount(index)"
                ...
                <div class="bg-white rounded-lg shadow-sm p-6 border border-red-200">
                    ...
                    <div class="bg-red-50 border border-red-200 rounded-lg p-4 mb-4">
                        <div class="flex items-start">
                            ...
                            <div class="text-sm text-red-700">
                                ...
                                <p>Biaya operasional tidak akan muncul di invoice customer dan hanya digunakan untuk
                                    perhitungan profit internal.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div v-if="operationalCosts.length === 0"
                        ...
                        <div class="flex flex-col items-center">
                            ...
                            <p class="text-sm text-red-500">Klik tombol "Tambah Biaya Operasional" untuk menambah biaya
                                internal</p>
                        </div>
                    </div>

                    <div class="space-y-4">
                        <div v-for="(cost, index) in operationalCosts" :key="'op-cost-' + index"
                            class="border border-red-200 rounded-lg p-4 bg-red-50">
                            <div class="flex items-center justify-between mb-4">
                                <h4 class="font-medium text-red-800">Biaya Operasional {{ index + 1 }}</h4>
                                <button type="button" @click="removeOperationalCost(index)"
                                    class="text-red-600 hover:text-red-800">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-6 gap-4">
                                <div class="md:col-span-2">
                                    ...
                                    <input type="text" v-model="cost.description"
                                        ...
                                        placeholder="e.g., Biaya trucking internal" required />
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-red-700 mb-2">Qty</label>
                                    <input type="number" v-model="cost.quantity"
                                        @input="calculateOperationalAmount(index)" step="0.01" min="0.01"
                                        class="w-full px-3 py-2 border border-red-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500"
                                        required />
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-red-700 mb-2">Unit</label>
                                    <input type="text" v-model="cost.unit"
                                        class="w-full px-3 py-2 border border-red-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500"
                                        placeholder="SET, KG, dll" required />
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-red-700 mb-2">Rate</label>
                                    <input type="number" v-model="cost.rate" @input="calculateOperationalAmount(index)"
                                        step="0.01" min="0"
                                        class="w-full px-3 py-2 border border-red-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500"
                                        required />
                                </div>
                                <div>
                                    ...
                                    <input type="text" :value="formatCurrency(cost.amount || 0)"
                                        class="w-full px-3 py-2 border border-red-300 rounded-lg bg-red-100" readonly />
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div v-if="operationalCosts.length > 0" class="mt-6 bg-red-50 border border-red-200 rounded-lg p-4">
                        <div class="flex justify-between items-center">
                            <span class="text-sm font-medium text-red-700">Total Biaya Operasional:</span>
                            <span class="text-lg font-bold text-red-800">{{ formatCurrency(calculateOperationalTotal())
                                }}</span>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div class="bg-green-50 rounded-lg p-4 border border-green-200">
                            <div class="text-center">
                                <div class="text-xl font-bold text-green-800">{{ formatCurrency(calculateGrossRevenue())
                                    }}</div>
                        ...
                        <div class="bg-red-50 rounded-lg p-4 border border-red-200">
                            <div class="text-center">
                                <div class="text-xl font-bold text-red-800">{{
                                    formatCurrency(calculateOperationalTotal()) }}</div>
                        ...
                        <div class="bg-blue-50 rounded-lg p-4 border border-blue-200">
                            <div class="text-center">
                                <div class="text-xl font-bold text-blue-800">{{ formatCurrency(calculateNetProfit()) }}
                                </div>
...
const normalizeVendorId = (value) => {
    if (value === null || value === undefined || value === '' || value === 'internal') {
        return null
    }
...
const submit = () => {
    // Combine all items from all three arrays with proper type and flags
    const allItems = [
