<template>
  <AdminKeuanganLayout>
    <Head title="Edit Operational Cost Category" />
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="mb-6">
          ...
          >
            ...
            Back to Operational Cost Categories
          </Link>
          <h1 class="text-2xl font-bold text-gray-900">Edit Operational Cost Category</h1>
          <p class="mt-1 text-sm text-gray-600">Update category {{ operationalCostCategory.name }}</p>
        </div>
...
const indexUrl = computed(() =>
  `${route('admin-keuangan.operational-cost-categories.index')}${buildQueryString(props.filters || {})}`
);

const updateUrl = computed(() =>
  `${route('admin-keuangan.operational-cost-categories.update', props.operationalCostCategory.id)}${buildQueryString(props.filters || {})}`
);
