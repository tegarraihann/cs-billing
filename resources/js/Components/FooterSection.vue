<template>
  ...
  >
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-20 lg:py-24">
      ...
      <div
        class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-12 lg:gap-16"
      >
        <!-- Company Info -->
        <div
        ...
        >
          <!-- Company Logo & Name -->
          <div class="mb-6">
            <h3 id="company-info-heading" class="sr-only">
              Company Information
            </h3>
            <div class="text-center mb-4">
              ...
              />
              <h3
                class="text-lg md:text-xl font-bold text-white font-inter whitespace-nowrap"
              >
          ...
          <p
            class="text-gray-300 mb-6 md:mb-8 leading-relaxed font-jost text-sm md:text-base"
          >
            Reliable logistics solutions for export-import, inland trucking,
            international money transfer, and shipment insurance with high
            professional standards since 2004.
          </p>
          ...
          <div class="flex flex-wrap items-center gap-2 md:gap-3 mb-6 md:mb-8">
            ...
            >
              ...
              ></div>
              <span
                class="text-xs md:text-sm text-sage-200 font-jost font-medium"
                >Online 24/7</span
              >
            </div>
            <div
            ...
            >
              <svg
                class="w-3 h-3 text-sage-300 mr-2"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
          ...
          >
            ...
            >
              <div
                v-html="social.icon"
                class="w-4 h-4 md:w-5 md:h-5 text-gray-300 group-hover:text-white"
              ></div>
            </a>
          </div>
        </div>
        ...
        <div role="navigation" aria-labelledby="services-heading">
          <h3
            id="services-heading"
            class="text-lg md:text-xl font-bold mb-6 md:mb-8 text-white font-inter"
          >
            Our Services
          </h3>
          <ul class="space-y-3 md:space-y-4" role="list">
            <li v-for="service in services" :key="service.name">
              <a
                :href="service.link"
                @click="scrollToSection(service.link)"
              ...
              >
                <svg
                  class="w-3 h-3 md:w-4 md:h-4 mr-2 md:mr-3 mt-1 flex-shrink-0 text-sage-600 group-hover:text-sage-400 transition-colors duration-300"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                ...
                <span
                  class="group-hover:translate-x-1 transition-transform duration-300 font-jost text-sm md:text-base"
                  >{{ service.name }}</span
                >
              </a>
            </li>
        ...
        <div role="navigation" aria-labelledby="quick-links-heading">
          ...
          <ul class="space-y-3 md:space-y-4" role="list">
            <li v-for="link in quickLinks" :key="link.name">
              <a
                :href="link.url"
                @click="scrollToSection(link.url)"
                class="text-gray-300 hover:text-sage-300 transition-colors duration-300 flex items-start group cursor-pointer"
        ...
        <div role="complementary" aria-labelledby="contact-info-heading">
          ...
          <div class="space-y-4 md:space-y-6" role="list">
            <div
              ...
              :key="contact.type"
              class="group hover:bg-white/5 p-3 rounded-xl transition-colors duration-300"
              role="listitem"
            >
              <div class="flex items-start">
                ...
                >
                  <div
                    v-html="contact.icon"
                    class="w-4 h-4 md:w-5 md:h-5 text-sage-300 group-hover:text-white"
                  ></div>
                </div>
                <div class="min-w-0 flex-1">
                  <div
                    class="text-xs md:text-sm text-sage-300 mb-1 font-medium font-jost"
                  >
                    {{ contact.type }}
                  </div>
                  <div
                    class="text-sm md:text-base text-white font-semibold mb-1 font-jost break-words"
                  >
                    {{ contact.value }}
                  </div>
                  <div
                    v-if="contact.secondary"
                    class="text-xs md:text-sm text-gray-400 font-jost"
                  >
                    {{ contact.secondary }}
                  </div>
                </div>
    ...
    <div class="border-t border-gray-700/50 bg-gray-900/50">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-8">
        <div
          class="flex flex-col lg:flex-row justify-between items-center space-y-4 lg:space-y-0 gap-4"
        >
          <div
            class="flex flex-col md:flex-row items-center space-y-3 md:space-y-0 md:space-x-6 text-center md:text-left"
          >
          ...
          <div class="flex items-center justify-center lg:justify-end">
            <div class="flex flex-col sm:flex-row items-center gap-3 sm:gap-4">
              <span class="text-xs md:text-sm text-gray-400 font-jost"
                >Certified by:</span
              >
              <div class="flex flex-wrap justify-center gap-2">
                ...
                >
                  <div
                    v-html="cert.icon"
                    class="w-3 h-3 md:w-4 md:h-4 text-sage-300 mr-1.5 md:mr-2"
                  ></div>
                  <span class="text-xs text-sage-200 font-medium font-jost">{{
                    cert.name
                  }}</span>
                </div>
              </div>
    ...
    <div class="fixed bottom-6 right-6 md:bottom-8 md:right-8 z-50">
      ...
      <div
        v-show="isExpanded"
        class="flex flex-col items-center space-y-3 mb-3 transition-all duration-300"
      >
        <div
          v-for="cs in csContacts"
          :key="cs.id"
          class="relative flex items-center group"
        >
          <!-- CS Name Badge - positioned to the left of button, shows on hover -->
          <div
            class="absolute right-14 bg-green-500/90 backdrop-blur-sm text-white px-3 py-1 rounded-full text-xs font-semibold shadow-lg opacity-0 group-hover:opacity-100 transition-all duration-300 whitespace-nowrap"
          >
            {{ cs.name }}
          </div>
          <!-- CS WhatsApp Button -->
          <button
            @click="openWhatsApp(cs.phone)"
      ...
      <div
        v-show="!isExpanded"
        class="absolute -left-32 top-1/2 transform -translate-y-1/2"
      >
        <div
          class="flex items-center bg-green-500/90 backdrop-blur-sm text-white px-3 py-2 rounded-full shadow-lg animate-pulse-slow"
        >
          <svg
            class="w-4 h-4 mr-2"
            fill="none"
        ...
        <div
          class="absolute right-0 top-1/2 transform translate-x-2 -translate-y-1/2"
        >
          <div
            class="w-0 h-0 border-l-8 border-l-green-500/90 border-t-4 border-t-transparent border-b-4 border-b-transparent"
          ></div>
        </div>
      </div>
