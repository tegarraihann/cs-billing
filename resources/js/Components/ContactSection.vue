<template>
  <section id="contact" class="py-24 bg-sage-900" aria-labelledby="contact-heading">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="text-center mb-16 lg:mb-24">
        ...
        <p class="text-lg md:text-xl text-white/90 max-w-3xl mx-auto leading-relaxed font-jost">
          Our team of experts is ready to help provide the best logistics solutions for your business with 24/7 service.
        </p>
      </div>
      ...
      <div class="grid lg:grid-cols-2 gap-8 md:gap-12 lg:gap-20 items-start">
        ...
        <div class="bg-white/10 backdrop-blur-md rounded-3xl p-8 md:p-10 lg:p-12 shadow-2xl border border-white/20 hover:bg-white/15 transition-all duration-300">
          ...
          <form @submit.prevent="submitForm" class="space-y-4 md:space-y-6" aria-labelledby="contact-form-heading" novalidate>
            <div class="grid md:grid-cols-2 gap-4 md:gap-6">
              <div>
                <label for="contact-name" class="block text-xs md:text-sm font-semibold text-white/90 mb-2 md:mb-3 font-jost">Full Name *</label>
                <input
                  id="contact-name"
                  v-model="form.name"
                  type="text"
                  ...
                  @blur="validateForm"
                />
                <p v-if="formErrors.name" id="name-error" class="text-red-300 text-xs mt-1 font-jost" role="alert">{{ formErrors.name }}</p>
              </div>
              <div>
                <label for="contact-email" class="block text-xs md:text-sm font-semibold text-white/90 mb-2 md:mb-3 font-jost">Email *</label>
                <input
                  id="contact-email"
                  v-model="form.email"
                  type="email"
                  ...
                  @blur="validateForm"
                />
                <p v-if="formErrors.email" id="email-error" class="text-red-300 text-xs mt-1 font-jost" role="alert">{{ formErrors.email }}</p>
              </div>
            </div>

            <div class="grid md:grid-cols-2 gap-4 md:gap-6">
              <div>
                <label for="contact-phone" class="block text-xs md:text-sm font-semibold text-white/90 mb-2 md:mb-3 font-jost">Mobile Phone *</label>
                <input
                  id="contact-phone"
                  v-model="form.phone"
                  type="tel"
                  ...
                  @blur="validateForm"
                />
                <p v-if="formErrors.phone" id="phone-error" class="text-red-300 text-xs mt-1 font-jost" role="alert">{{ formErrors.phone }}</p>
              </div>
              <div>
                <label for="contact-service" class="block text-xs md:text-sm font-semibold text-white/90 mb-2 md:mb-3 font-jost">Services</label>
                <select
                  id="contact-service"
                ...
                >
                  ...
                  <option value="money-transfer" class="text-gray-900">Transfer Uang</option>
                  <option value="insurance" class="text-gray-900">Asuransi Barang</option>
                  <option value="warehousing" class="text-gray-900">Warehousing</option>
            ...
            <div>
              <label for="contact-message" class="block text-xs md:text-sm font-semibold text-white/90 mb-2 md:mb-3 font-jost">Your Message *</label>
              <textarea
                id="contact-message"
                ...
                @blur="validateForm"
              ></textarea>
              <p v-if="formErrors.message" id="message-error" class="text-red-300 text-xs mt-1 font-jost" role="alert">{{ formErrors.message }}</p>
            </div>

            <!-- Success/Error Messages -->
            <div v-if="submitStatus === 'success'" class="mb-4 p-3 bg-green-500/20 border border-green-400/30 rounded-xl text-green-300 text-sm font-jost flex items-center">
            ...
            >
              ...
              <span v-else class="flex items-center justify-center">
                <svg class="animate-spin -ml-1 mr-3 h-5 w-5 text-sage-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
        ...
        <div class="space-y-6 md:space-y-8" role="complementary" aria-labelledby="contact-info-heading">
          <!-- Contact Info Cards -->
            ...
            <div v-for="office in offices" :key="office.id" class="bg-white/10 backdrop-blur-md rounded-2xl p-4 md:p-6 border border-white/20 hover:bg-white/15 transition-all duration-300">
              ...
              <div class="space-y-2 md:space-y-3">
                <div class="flex items-start">
                  ...
                  <span class="text-white/90 text-xs md:text-sm leading-relaxed font-jost">{{ office.address }}</span>
                </div>
                <div class="flex items-center">
                  <svg class="w-4 h-4 md:w-5 md:h-5 text-white/80 mr-2 md:mr-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <span class="text-white font-medium text-xs md:text-sm font-jost">{{ office.phone }}</span>
                </div>
                <div class="flex items-center">
                  <svg class="w-4 h-4 md:w-5 md:h-5 text-white/80 mr-2 md:mr-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M3 8l7.89 4.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>
                  </svg>
                  <span class="text-white font-medium text-xs md:text-sm break-all font-jost">{{ office.email }}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="bg-white/10 backdrop-blur-md rounded-2xl p-4 md:p-6 border border-white/20" role="complementary" aria-labelledby="why-choose-us-heading">
            ...
            <div class="space-y-3 md:space-y-4">
              <div v-for="reason in whyChooseUs" :key="reason.title" class="flex items-start group">
                ...
                <div>
                  <h5 class="font-semibold text-white text-xs md:text-sm mb-1 font-jost">{{ reason.title }}</h5>
                  <p class="text-white/80 text-xs leading-relaxed font-jost">{{ reason.description }}</p>
                </div>
              </div>
            </div>
          </div>
...
.bg-sage-900 { background-color: #5a7d52; }
