<template>
  <div
    class="mx-auto max-w-sm px-4 py-20 font-sans antialiased md:max-w-2xl md:px-8"
  >
    <div class="flex h-96 w-full flex-col items-center justify-center">
      <div class="relative flex h-full w-full items-center justify-center">
        ...
        <div class="relative flex flex-col items-center justify-center">
          <div class="relative w-80 h-80">
              ...
              >
                ...
                >
                  ...
                  <p class="text-white/90 text-sm flex items-center">
                    <template
                      v-if="
                        testimonial.designation.includes('+62') ||
                        testimonial.designation.includes('0895')
                      "
                    >
                      <span class="mr-2">{{
                        testimonial.designation.split("|")[0].trim()
                      }}</span>
                      <span class="flex items-center">
                        <svg
                          class="w-4 h-4 mr-1 text-white/90"
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                        >
            ...
            <!-- Floating Elements -->
            <div
              class="absolute -top-4 -right-4 w-6 h-6 bg-sage-400 rounded-full opacity-60 animate-pulse"
            ></div>
            <div
              class="absolute -bottom-6 -left-6 w-8 h-8 bg-sage-300 rounded-full opacity-40"
            ></div>
            <div
              class="absolute top-1/2 -right-8 w-4 h-4 bg-sage-500 rounded-full opacity-50"
            ></div>
          </div>
        </div>
      </div>
      ...
      <div class="flex flex-row items-center justify-center gap-4 py-6">
        ...
        >
          <ChevronLeftIcon
            class="h-6 w-6 text-sage-700 transition-transform duration-200"
  ...
  if (relativePosition === 0) {
    ...
    return {
      transform: `rotate(${getCardRotation(
        index
      )}deg) scale(1) translateY(0px) translateX(0px)`,
      zIndex: 10,
      opacity: 1,
    };
    ...
    return {
      transform: `rotate(${getCardRotation(
        index
      )}deg) scale(0.8) translateY(20px) translateX(${
        relativePosition > 0 ? 30 : -30
      }px)`,
      zIndex: 1,
      opacity: 0,
    };
...
.layered-card-shadow {
  box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.25),
    0 10px 20px -5px rgba(0, 0, 0, 0.1), 0 0 0 1px rgba(255, 255, 255, 0.1);
}
