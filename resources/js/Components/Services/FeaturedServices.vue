  ...
  <div class="grid lg:grid-cols-2 gap-8">
    ...
    >
      ...
      <div
        ...
        :style="
          featured.image_path
            ? `background-image: url('${getServiceImageUrl(
                featured.image_path
              )}')`
      ...
      >
        <div
          class="absolute inset-0 bg-gradient-to-br from-black/60 via-black/40 to-black/70 group-hover:from-black/50 group-hover:to-black/60 transition-all duration-500"
        ></div>
      </div>

      <!-- Content Overlay -->
      <div
        class="relative z-10 p-8 h-full flex flex-col justify-between min-h-[500px]"
      >
        <!-- Header -->
        <div>
          <div class="flex items-center gap-4 mb-6">
            <div
              class="w-14 h-14 bg-sage-600/90 backdrop-blur-sm rounded-2xl flex items-center justify-center group-hover:bg-sage-600 transition-colors duration-300"
            >
              <component
                :is="getServiceIcon(featured)"
                class="w-7 h-7 text-white"
              />
            </div>
            <div
              class="bg-sage-600 text-white px-4 py-2 rounded-full text-sm font-bold shadow-lg"
            >
              Featured Service
            </div>
          </div>
        ...
        <div v-if="featured.features?.length" class="mb-8">
          <div class="bg-black/20 backdrop-blur-sm rounded-2xl p-6">
            ...
            <div class="space-y-3">
              ...
              >
                <div
                  class="w-6 h-6 bg-sage-500/30 rounded-full flex items-center justify-center mt-0.5 flex-shrink-0"
                >
    ...
    <div class="space-y-6">
      ...
      >
        <!-- Background Pattern -->
        <div
          class="absolute top-0 right-0 w-32 h-32 opacity-5 group-hover:opacity-10 transition-opacity duration-300"
        >
          <div
            class="w-full h-full bg-sage-600 rounded-full transform translate-x-8 -translate-y-8"
          ></div>
        </div>
        ...
        <div
          ...
          :style="`background-image: url('${getServiceImageUrl(
            service.image_path
          )}');`"
        ></div>
        ...
        <div class="relative flex items-start gap-4">
          <div
            class="w-12 h-12 bg-sage-100 group-hover:bg-sage-600 rounded-2xl flex items-center justify-center transition-colors duration-300 flex-shrink-0"
          >
            <component
              :is="getServiceIcon(service)"
              class="w-6 h-6 text-sage-600 group-hover:text-white transition-colors duration-300"
            />
          </div>

          <div class="flex-1 min-w-0">
            <h4
              class="text-lg font-bold text-gray-900 group-hover:text-sage-700 transition-colors duration-300 mb-2"
            >
              {{ service.title }}
            </h4>
          ...
          <div class="flex-shrink-0">
            <ArrowRightIcon
              class="w-5 h-5 text-gray-400 group-hover:text-sage-600 transition-colors duration-300"
            />
          </div>
        </div>
      </div>
