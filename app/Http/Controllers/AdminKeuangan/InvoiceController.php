...
class InvoiceController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($isAdditional && isset($validated['items']) && is_array($validated['items'])) {
            $filteredItems = $this->filterAdditionalInvoiceManualItems($validated['items']);
            $removedCount = count($validated['items']) - count($filteredItems);

            if ($removedCount > 0) {
                \Log::warning('Additional invoice auto-synced items removed before create', [
                    'sales_order_id' => $salesOrder->id,
                    'base_invoice_id' => $validated['base_invoice_id'] ?? null,
        ...
        if ($isAdditional && $this->hasDuplicateAdditionalInvoice($salesOrder, $validated)) {
            return back()->withErrors([
                'items' => 'A similar additional invoice already exists for this Sales Order. Please review the existing additional invoice before creating a new one.',
            ])->withInput();
        }
    ...
    private function filterAdditionalInvoiceManualItems(array $items): array
    {
        return array_values(array_filter($items, function ($item): bool {
            return !$this->isAutoSyncedAdditionalInvoiceItem($item);
        }));
    }

    private function isAutoSyncedAdditionalInvoiceItem($item): bool
    {
        if (!is_array($item)) {
            return true;
        }
        ...
        $autoGenerated = filter_var($item['auto_generated'] ?? false, FILTER_VALIDATE_BOOLEAN);

        if ($itemRef !== '' && (
            str_starts_with($itemRef, 'other_cost_') ||
            str_starts_with($itemRef, 'vendor_') ||
            str_starts_with($itemRef, 'cogs_vendor_') ||
            str_starts_with($itemRef, 'ap_component_')
        )) {
            return true;
        }

        return $autoGenerated && in_array($source, [
            'sales_order_other_costs',
            'vendor_breakdown_buying',
            'sales_order_vendor_breakdown',
            'account_payable_component',
        ], true);
    }

    private function hasDuplicateAdditionalInvoice(SalesOrder $salesOrder, array $validated): bool
    {
        $baseInvoiceId = $validated['base_invoice_id'] ?? null;
        $candidateSignature = $this->additionalInvoiceCustomerItemSignature($validated['items'] ?? []);

        if (empty($candidateSignature)) {
            return false;
        }
        ...
        $existingInvoices = Invoice::query()
            ...
            ->where('sales_order_id', $salesOrder->id)
            ->where('is_additional', true)
            ->when($baseInvoiceId, fn ($query) => $query->where('base_invoice_id', (int) $baseInvoiceId))
            ->get();

        foreach ($existingInvoices as $invoice) {
            if ($this->additionalInvoiceCustomerItemSignature($invoice->items->all()) === $candidateSignature) {
                \Log::warning('Duplicate additional invoice creation blocked', [
                    'sales_order_id' => $salesOrder->id,
                    'base_invoice_id' => $baseInvoiceId,
                    'existing_invoice_id' => $invoice->id,
                    'existing_invoice_number' => $invoice->invoice_number,
                    'user_id' => auth()->id(),
                ]);
    ...
    private function additionalInvoiceCustomerItemSignature(array $items): array
    {
        $signature = [];

        foreach ($items as $item) {
            $itemType = strtolower(trim((string) (is_array($item) ? ($item['item_type'] ?? 'billable') : ($item->item_type ?? 'billable'))));
            $includeInCustomerInvoice = is_array($item)
                ? filter_var($item['include_in_customer_invoice'] ?? true, FILTER_VALIDATE_BOOLEAN)
                : (bool) ($item->include_in_customer_invoice ?? true);
            $isHiddenFromCustomer = is_array($item)
                ? filter_var($item['is_hidden_from_customer'] ?? false, FILTER_VALIDATE_BOOLEAN)
                : (bool) ($item->is_hidden_from_customer ?? false);

            if ($itemType === 'operational_cost' || !$includeInCustomerInvoice || $isHiddenFromCustomer) {
                continue;
            }

            $quantity = (float) (is_array($item) ? ($item['quantity'] ?? 1) : ($item->quantity ?? 1));
            $rate = (float) (is_array($item) ? ($item['rate'] ?? 0) : ($item->rate ?? 0));
            $amount = (float) (is_array($item) ? ($item['amount'] ?? ($quantity * $rate)) : ($item->amount ?? ($quantity * $rate)));
            $unit = strtoupper(trim((string) (is_array($item) ? ($item['unit'] ?? 'SET') : ($item->unit ?? 'SET'))));

            $signature[] = [
                'item_type' => $itemType ?: 'billable',
        ...
        usort($signature, static function (array $left, array $right): int {
            return [$left['item_type'], $left['unit'], $left['quantity'], $left['amount']]
                <=> [$right['item_type'], $right['unit'], $right['quantity'], $right['amount']];
        });
    ...
    private function cleanupAdditionalInvoiceAutoSyncedItems(Invoice $invoice): void
    {
        $deletedCount = InvoiceItem::where('invoice_id', $invoice->id)
            ->where(function ($q) {
                $q->where('item_ref', 'like', 'vendor_%')
                    ->orWhere('item_ref', 'like', 'cogs_vendor_%')
                    ->orWhere('item_ref', 'like', 'other_cost_%')
                    ->orWhere('item_ref', 'like', 'ap_component_%')
                    ->orWhere(function ($sourceQuery) {
                        $sourceQuery->where('item_type', 'operational_cost')
                            ->where(function ($refQuery) {
                                $refQuery->whereNull('item_ref')
                                    ->orWhere('item_ref', 'operational_cost');
                            });
                    });
            })
            ->delete();
        ...
        if ($deletedCount > 0) {
            ...
            $receivable = $invoice->accountReceivable()->first();
            if ($receivable) {
                $receivable->syncComponentsFromInvoice($invoice);
            }

            app(InvoicePostingService::class)->sync($invoice->fresh(['items']));
        }
    }
